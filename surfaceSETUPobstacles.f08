...
do time = 1,maxtime
 ...
 subroutine foundation
   integer::maxl,speciesselect,beadcounter
...
!if(beadcounter>(gridsize**2)) then
!write(6,*) 'FAIL: too many binding sites on surface'
!STOP 55
!end if
...
if(m<=speciespop(1)) then
   speciesselect = 1
else if (m>speciespop(1)) then
        speciesselect=2
end if
...
do l = 1,maxl,1
   ...
   do m = speciespop(1)+1,speciespop(1)+speciespop(2)
