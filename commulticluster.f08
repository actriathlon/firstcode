program move
  ...
  integer :: successful,reject,pivotlimit,maxpiv,maxlength1,maxlength2,q,d,u,sm,qt,ft,gt
...
!write(6,*) 'c'
     ...
     if ((mod(time,1000000) == 0) .and. (film .eqv. .true.) ) then
        call dataout
     end if
     !end if
     if((nobonds .eqv. .true.) .and. (clt .eqv. .false.) .and. (clr .eqv. .false.)) then
        !write(6,*) 'lies',nobonds,clt,clr
call positioning
else
    !write(6,*) 'not lies'
     call pickmoves
...
!call clustercount
     ...
     !else if(modulo(time,10000) == 0) then
        !write(6,*) 'step =',time
     end if
...
subroutine vectorspin(m,l,maxl)
       ...
       do bead1 = beadmin,beadmax,1
          ...
          do bead2 = 1,maxback,1
             ...
             if(br(chain2,chain1,bead2,bead1) == 1) deltaenergy = deltaenergy - &
                  interen(protcoords(chain1,bead1)%species,protcoords(chain2,bead2)%species)
  ...
  subroutine intrabondvector(chain1,bead1,tempcoord,tempisobond,deltaenergy,tbo,maxl)
    ...
    logical :: adjver
       ...
       do bead2 = 1,bead1-3
          if(isobond(chain1,bead2,bead1) == 1) deltaenergy = deltaenergy - intraenergy(protcoords(chain1,bead2)%species)
          call adjacent(chain1,bead2,bead1,tempcoord,adjver,bdir)
          if ((adjver.eqv. .true.) .and. &
               (bdir == -1*tbo(bead2)) .and. (bdir == tbo(bead1))) then
             deltaenergy = deltaenergy + intraenergy(protcoords(chain1,bead2)%species)
...
17     if(protcoords(m,1)%species == 1) maxl = maxlength1
       if(protcoords(m,1)%species == 2) maxl = maxlength2
       !write(6,*) maxl
             ...
             do f = 1,maxlengthss
                ...
                if(adjver.eqv. .true.) then
                   if(isbond .eqv. .true.) then
                      if(interen(protcoords(m,l)%species,protcoords(g,f)%species) < 0.0) then
                      cb(m,g) = 1
                      cb(g,m) = 1
    ...
    if (choose > 0.0) then
       ...
       do pr = 1,m-1,1
          ...
          do st = 1,maxback
             ...
             if(br(pr,m,st,maxl) == 1) deltaenergy = deltaenergy - interen(protcoords(m,1)%species,protcoords(pr,st)%species)
             call adjacent(pr,1,st,tempcoord,adjver,bdir)
             if((adjver.eqv. .true.)) then
                deltaenergy = deltaenergy + interen(protcoords(m,1)%species,protcoords(pr,st)%species)
                tbr(pr,1,st) = 1
       ...
       do pr = m+1,nprotein,1
          ...
          do st = 1,maxback
             ...
             if(br(m,pr,maxl,st) == 1) deltaenergy = deltaenergy -&
                  interen(protcoords(m,1)%species,protcoords(pr,st)%species)
             call adjacent(pr,1,st,tempcoord,adjver,bdir)
             if((adjver.eqv. .true.)) then
                deltaenergy = deltaenergy + interen(protcoords(m,1)%species,protcoords(pr,st)%species)
                tbr(pr,1,st) = 1
       ...
       do pr = 1,m-1,1
          ...
          do st = 1,maxback,1
             if(intbond(pr,m,st,1) == 1) deltaenergy = deltaenergy - &
                  interenergy(protcoords(m,1)%species,protcoords(pr,st)%species)
             if(br(pr,m,st,1) == 1) deltaenergy = deltaenergy - interen(protcoords(m,1)%species,protcoords(pr,st)%species)
       ...
       do pr = m+1,nprotein,1
          ...
          do st = 1,maxback,1
             if(intbond(m,pr,1,st) == 1) deltaenergy = deltaenergy - &
                  interenergy(protcoords(m,1)%species,protcoords(pr,st)%species)
             if(br(m,pr,1,st) == 1) deltaenergy = deltaenergy -interen(protcoords(m,1)%species,protcoords(pr,st)%species)
             call adjacent(pr,maxl,st,tempcoord,adjver,bdir)
             if((adjver.eqv. .true.)) then
                deltaenergy = deltaenergy + interen(protcoords(m,1)%species,protcoords(pr,st)%species)
                tbr(pr,maxl,st) = 1
  ...
  subroutine clustercom(m,clsize,comcluster,cb,cllist,route,rcounter,totcluspop,checklist,toolarge)
    ...
    do a = 1,clsize-1,1
       ...
       do g = a+1,clsize
          ...
          comz(f,b) = 0.0d0
          if(cb(b,f)==1) then
             if(protcoords(b,1)%species == 1) maxb = maxlength1
             if(protcoords(b,1)%species == 2) maxb = maxlength2
             if(protcoords(f,1)%species == 1) maxltest = maxlength1
             if(protcoords(f,1)%species == 2) maxltest = maxlength2
...
centralchain = m
    ...
    if(protcoords(dint,1)%species == 1) maxl = maxlength1
    if(protcoords(dint,1)%species == 2) maxl = maxlength2
             ...
             do f = 1,maxlengthss,1
                !if(g /=m) then
                   ...
                   if((adjver .eqv. .true.)) then
                      if(isbond .eqv. .true.) then
                         br(m,g,l,f) = 1
                         initialenergy = initialenergy + interen(protcoords(m,l)%species,protcoords(g,f)%species)
                      end if
          ...
          if(clno(a) /= clno(g)) then
             ...
             do l = 1,maxl,1
                ...
                do f = 1,maxlengthss,1
                   call adjacent(g,l,f,tempcoord,adjver,bdir)
                   if(adjver.eqv. .true.) then
                      if(isbond .eqv. .true. .and. (interen(protcoords(a,l)%species,protcoords(g,f)%species) < 0.0)) then
       ...
       b = cllist(a)
             do g = a+1,clsize
                ...
                if(cb(b,f)==1) then
                   if(protcoords(b,1)%species == 1) maxb = maxlength1
                   if(protcoords(b,1)%species == 2) maxb = maxlength2
                   if(protcoords(f,1)%species == 1) maxl = maxlength1
                   if(protcoords(f,1)%species == 2) maxl = maxlength2
...
31  if(clusmove .eqv. .false.) moveallow = .false.
  ...
  subroutine clustercount
    integer :: m,l,g,f,clcount,clusterpop,bdir,maxlengthss,maxback,clustcount,a,normalisedsize
    integer,dimension(:),allocatable:: clnos
    logical :: clusyes,adjver,newcl
    type(basicp),dimension(:),allocatable :: tempcoord
    integer,dimension(:),allocatable:: cllist,histcl,mashist
    integer :: oldcl,dum1,dum2,zzz,maxclus
...
continue
   ...
   a = a+1
       ...
       if(clustcount /= 0 .and.  (time > equilib)) then
         avecluspop = avecluspop + clusterpop + clustcount
...
acount = 0
  ...
  SUBROUTINE read_setup
    ...
    DO
       ...
       CASE ('INTERENBB')
          CALL get_dp(interen(2,2))
          interen(2,2) = -ABS(interen(2,2))

       CASE ('INTERENAA')
          CALL get_dp(interen(1,1))
          interen(1,1) = -ABS(interen(1,1))


       CASE ('INTERENAB')
          CALL get_dp(interen(1,2))
          interen(1,2) = -ABS(interen(1,2))
          interen(2,1) = interen(1,2)
       ...
       CASE ('DIRECTAA')
          CALL get_dp(interenergy(1,1))
          interenergy(1,1) = -abs(interenergy(1,1))
 

       CASE ('DIRECTAB')
          CALL get_dp(interenergy(1,2))
          interenergy(1,2) = -abs(interenergy(1,2))
          interenergy(2,1) = interenergy(1,2)
          
       CASE ('DIRECTBB')
          CALL get_dp(interenergy(2,2))
          interenergy(2,2) = -abs(interenergy(2,2))
