program move
  ...
  integer :: successful,reject,pivotlimit,maxpiv
  integer:: cracc,crrej,raacc,rarej,rerej,reacc,pvacc,pvrej
...
83  if (reptcont .eqv. .false.) then
  ...
  subroutine energy
    integer :: dx,dy,dz,m,l,f,g,bdir,delx,dely,delz
...
!!!!!!! interatomic
 ...
 31   if(dx + dy + dz ==1) then
  ...
  subroutine dataout
    integer::m,l
    type(protein),dimension(:,:),allocatable :: bondposit
    !write(6,*) 'data'
    allocate(bondposit(nprotein,maxlength))
    write(67,*) 2*nprotein*maxlength
    write(67,*) ' '
    t = time 
    do m = 1,nprotein,1
       do l = 1,maxlength,1
          write(67,*) 'C', 2*protcoords(m,l)%x, 2*protcoords(m,l)%y, &
               2*protcoords(m,l)%z
          bondposit(m,l)%x = protcoords(m,l)%x
          bondposit(m,l)%y = protcoords(m,l)%y
           bondposit(m,l)%z = protcoords(m,l)%z

          if(bonddd(m,l) == 1) bondposit(m,l)%x = modulo(bondposit(m,l)%x + 1,gridsize)
          if(bonddd(m,l) == -1) bondposit(m,l)%x = modulo(bondposit(m,l)%x - 1,gridsize)
          if(bonddd(m,l) == 1) bondposit(m,l)%y = modulo(bondposit(m,l)%y + 1,gridsize)
          if(bonddd(m,l) == -1) bondposit(m,l)%y = modulo(bondposit(m,l)%y - 1,gridsize)
          if(bonddd(m,l) == 1) bondposit(m,l)%z = modulo(bondposit(m,l)%z + 1,gridsize)
          if(bonddd(m,l) == -1) bondposit(m,l)%z = modulo(bondposit(m,l)%z - 1,gridsize)
...
39     if(clusmove .eqv. .true.) continue
  ...
  subroutine clustercount
    integer :: m,l,g,f,clcount,clusterpop,bdir
