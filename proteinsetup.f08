program setup
  ...
  integer :: i,j,k,gridsize,nprotein1,nprotein2
...
!write(19,*) 'a'
  ...
  call foundation
  !write(6,*) 'c'
  write(19,*) (nprotein1*maxlength1) + (nprotein2*maxlength2)
  write(19,*) ' '
...
contains
 ...
 subroutine positioning
   integer :: gate,xbk,ybk,zbk,protpass,f,speciesselect,spcount1,spcount2
   Real:: t1,t2,t3,t4,t5,t6
   do m = 1,nprotein1+nprotein2
      hold = m
...
47    if (protpass ==1) then
      ...
      speciesselect = int(ran2(seed)*2)+1

      if(speciesselect ==1) maxlength = maxlength1
      if(speciesselect ==2) maxlength = maxlength2 
      
      if(spcount1 == nprotein1) maxlength = maxlength2
      if(spcount2 == nprotein2) maxlength = maxlength1
...
15          if (control ==1) then
               ...
               protcoords(m,l)%species = speciesselect
               !protcoords(m,l)%location = section               
               if(speciesselect ==1) spcount1 = spcount1 + 1
               if(speciesselect ==2) spcount2 = spcount2 + 1
          ...
          do l = 1,maxlength
             write(19,*) protcoords(m,l)%species ,    protcoords(m,l)%x,    protcoords(m,l)%y,    protcoords(m,l)%z
          end do
       end do
