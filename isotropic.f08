...
71  if(cranksep .eqv. .true.) then
    ...
    if (pivotx .eqv. .true.) then
       ...
       do s= l,p,1
          ...
          dy1(s) = protcoords(m,p)%y - protcoords(m,l)%y
          if(dy1(s) > gridsize/2) dy1 = -1.0
          if(dy1(s) < -gridsize/2) dy1 = 1.0
          
          
          dz1(s) = protcoords(m,p)%z - protcoords(m,l)%z
          if(dz1(s) > gridsize/2) dz1 = -1.0
          if(dz1(s) < -gridsize/2) dz1 = 1.0
                ...
                do f = 1,maxlength,1
                   if(g /= m .or. f <= l .or. f >= p) then
                      if(protcoords(g,f)%y == tempcoord(m,s)%y.and. &
                           protcoords(g,f)%x == protcoords(m,s)%x .and. &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!111
       ...
       do s= l,p,1
          ...
          dx1(s) = protcoords(m,p)%x - protcoords(m,l)%x
          if (dx1(s) > gridsize/2) dx1 = -1.0
          if (dx1(s) < -gridsize/2) dx1 = 1.0
          dz1(s) = protcoords(m,p)%z - protcoords(m,l)%z
          if (dz1(s) > gridsize/2) dz1 = -1.0
          if (dz1(s) < -gridsize/2) dz1 = 1.0
                ...
                do f = 1,maxlength,1
                   if(g /= m .or. f <= l .or. f >= p) then
                      if(  protcoords(g,f)%x == tempcoord(m,s)%x.and. &
                           protcoords(g,f)%y == protcoords(m,s)%y .and. &
                           protcoords(g,f)%z == tempcoord(m,s)%z) then
                         crankcont = .false.
                         goto 43
                      else
    ...
    !!pivot y!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
             ...
             do s = l,p,1
                do g = 1,nprotein,1
                   do f = 1,maxlength,1
                      if(g /= m .or. f <= l .or. f >= p) then
                         if(  protcoords(g,f)%y == tempcoord(m,s)%y.and. &
                              protcoords(g,f)%z == protcoords(m,s)%z .and. &
       ...
       do s = l,p,1
          do g = 1,maxlength,1
             if(g < l .or. g > p) then
                tempisobond(m,s,g) = protcoords(m,s)%x - protcoords(m,g)%x + &
                     protcoords(m,l)%y - protcoords(m,g)%y + protcoords(m,s)%z - protcoords(m,g)%z
                if( modulo(tempisobond(m,s,g)-isobond(m,s,g),gridsize) == 1.0) deltaiso = deltaiso + 1
                if(isobond(m,s,g)== 1.0 .and. modulo(tempisobond(m,s,g),gridsize) /= 1.0) deltaiso = deltaiso - 1
             end if
...
!       if(deltaiso > 0.0 .or. ran2(seed) > 0.01) then
    ...
    if (rightangle .eqv. .true.) then    
       ...
       if (abs(di) <= 1 .or. abs(dj) <= 1 .or. abs(dk) <= 1) then
          ...
          do g = 1, nprotein,1
             ...
             do g = 1,maxlength,1
                if(g /= l) then
           tempisobond(m,l,g) = i - protcoords(m,g)%x + &
                             j - protcoords(m,g)%y + k - protcoords(m,g)%z
           if( modulo(tempisobond(m,l,g)-isobond(m,l,g),gridsize) == 1.0) deltaiso = deltaiso + 1
           if(isobond(m,l,g)== 1.0 .and. modulo(tempisobond(m,l,g),gridsize) /= 1.0) deltaiso = deltaiso - 1
           end if
     end do


  !   if(deltaiso > 0 .or. ran2(seed) > 0.01) then
        protcoords(m,l)%x = i
        protcoords(m,l)%y = j
        protcoords(m,l)%z = k
...
!continue
    !else if (l == 1000) then
       ...
       do b = l+1,maxlength,1
           ...
           tempisobond(m,b,g) = protcoords(m,b)%x - protcoords(m,g)%x + &
                             protcoords(m,b)%y - protcoords(m,g)%y + protcoords(m,b)%z - protcoords(m,g)%z
           if( modulo(tempisobond(m,b,g)-isobond(m,b,g),gridsize) == 1.0) deltaiso = deltaiso + 1
           if(isobond(m,b,g)== 1.0 .and. modulo(tempisobond(m,b,g),gridsize) /= 1.0) deltaiso = deltaiso - 1
        end do
     end do
  ...
  subroutine reptation
    ...
    if(reptcont .eqv. .true.) then
             ...
             do g = 2,maxlength,1
                ...
                sumdebug = tempcoord(c,1)%x - protcoords(c,g-1)%x + &
                             tempcoord(c,1)%y - protcoords(c,g-1)%y + tempcoord(c,1)%z - protcoords(c,g-1)%z
       ...
       if(reptcont .eqv. .true.) then
          ...
          deltaiso = 0
           do g = 1,maxlength-1,1
                ...
                sumdebug = tempcoord(c,maxlength)%x - protcoords(c,g+1)%x + &
                tempcoord(c,maxlength)%y - protcoords(c,g+1)%y + tempcoord(c,maxlength)%z - protcoords(c,g+1)%z
...
83  if (reptcont .eqv. .false.) then
  ...
  !   if(deltaiso > 0 .or. ran2(seed) > exp(-deltaiso/3.0)) then    
