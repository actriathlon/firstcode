...
totalenergy=totalenergy+deltaenergy
  ...
  subroutine cleanup
    integer::m,l,maxl,a
integer,dimension(:),allocatable::scli,unbb
  allocate(scli(2*(clientlength+1)))
allocate(unbb(clientlength))
...
do l=1,clientlength,1
unbb(l)=unbb(l)+unbound(a,l)
end do

do l=1,2*(clientlength+1),1
scli(l)=scli(l)+sumclient(a,l)
end do

end do
       write(108,*) scli(:),sumsep/scli(clientlength+1),bulksumsep/scli(2*(clientlength+1)),unbb(:),&
