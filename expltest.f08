program move
  ...
  integer:: rerej,reacc,nprotein1,nprotein2,nspecies,mtype,freezetime,nclientspecies,nscaffold,tratt,tracc
  integer::count_rate,count_max,start,finish,midpoint
  integer:: pvacc,pvrej,rarej,raacc,cracc,crrej,vtype2
  integer,dimension(:,:),allocatable ::bonddd,study,sumclient,visit,visit2,unbound,bdi
...
integer::reptpreenforward,reptpreenback,reptattforward,reptattback,reptmoveforward,reptmoveback
integer,dimension(6)::trdir
integer,dimension(:),allocatable :: bindinghist,zloc
...
type(centremass),dimension(:),allocatable::initcom
...
reacc = 0
  ...
  totdire = 0.0
  totiso = 0.0d0
  do ft = 1,mtype-nclientspecies,1
     do gt = 1,mtype-nclientspecies,1
...
initcom(qt)%x = com(1,qt)%x
initcom(qt)%y = com(1,qt)%y
initcom(qt)%z = com(1,qt)%z
...
!write(6,*) 'c'
     ...
     if((real(modulo(midpoint-start,count_max))/count_rate)> 172800) then
        backtime = time
        goto 93
     end if
...
if(modulo(time,outputrate/1000)==0) then
      call analyse
end if
     
     if (mod(time,outputrate) == 0) then
        if(time==outputrate) then
        open(93, file = 'energy.dat', action = 'write')
else
        open(93,file='energy.dat',access='append')
end if
...
close(93)
     ...
     else if(modulo(time,1000000) == 0) then
        write(6,*) 'step =',time
	end if

if(modulo(time,outputrate/100) == 0) then

    write(9999,*) time,loc(:),locb(:)
	 end if
...
write(6,*) 'FORWARD', reptpreenforward,reptmoveforward,reptattforward
write(6,*) 'BACKWARD',reptpreenback,reptmoveback,reptattback
...
do l = 1,maxl
	if(protcoords(m,l)%type ==3) goto 54
...
if(protcoords(g,f)%x>gridsize) bindchoose = bindchoose +1
if(protcoords(g,f)%x<=gridsize) bindchoose = bindchoose -1
if(protcoords(g,f)%x>gridsize) locb(2) = locb(2) +1
if(protcoords(g,f)%x<=gridsize) locb(1) = locb(1) +1
...
!  if(com(1,m)%x >gridsize) then
!     loc(2) = loc(2)+1
!  else if(com(1,m)%x <gridsize) then
!     loc(1) = loc(1)+1
!     end if
...
if(control >0) then
	if(bindchoose <0) left = .true.
	if(bindchoose >0) left= .false.
	if(bindchoose ==0) then
	if(lc .eqv. .true.) left = .true.
	if(lc .eqv. .true.) left = .false.
	end if
...
if((study(a,g) == 1) .and. (protcoords(a,g)%type /= 3)) call rdf(m,g,left)
end do

end if
      if(control > 1) then

         dx = min(abs(protcoords(m,1)%x - protcoords(m,2)%x),gridsizex-abs(protcoords(m,1)%x&
              -protcoords(m,2)%x))
...
do g = 1,clientlength
if((study(a,g) ==0).and. (protcoords(a,g)%type /= 3)) unbound(a,g) =unbound(a,g)+1
end do
...
double precision:: direction
    ...
    logical :: rac,racc,adjver,moveallow
    integer:: st,pr,maxl,maxback,moved,f
...
call fastoverlap(m,maxl,moveallow,moved)
             ...
             do f = 1,maxl
                if(abs(f-l)>2) then
                   ...
                   if((adjver .eqv. .true.)) then
                      call bondformintra(m,l,f,tempcoord,tbo,bdir,deltaenergy)
                   end if
                end if
             end do
end do
...
trdir(moved) = trdir(moved)+1    
  ...
  subroutine fastoverlap(m,maxl,clusmove,moved)
...
if(time==0) then
        ...
        open(160,file='origdimens.dat',access='append')
end if

 
   !open(160,file='origdimens.dat',access='append')
!open(161,file='dimens.dat',access='append')
...
else if (m<nprotein) then

  call bonddirection2(tbo(l))
end if
...
if(protcoords(m,l)%fm /=0) then
 ...
 82 continue
    ...
    if(Energydecision(deltaenergy) .eqv. .true.) then
       totalenergy = totalenergy + deltaenergy
	if(m<nprotein) then
       call updateposold(m,l,l,tempcoord,tbo,maxl)
	else
tbdi(l) = bdi(m,l)
...
topchains = 0
  ...
  do n=3,nspecies-nclientspecies
	topchains = topchains+speciespop(n)
...
protcoords(m,1)%linker = specieslinker(protcoords(m,1)%type)
  ...
  subroutine setupclient(m)
    ...
    integer::xbk,ybk,zbk,maxback,no1,no2
...
35    if (gate == 1) then
         l = 1
         i = int(ran2(seed)*gridsizex)
         j = int(ran2(seed)*gridsize)
         k = int(ran2(seed)*gridsize)
         !write(6,*) 'e'
         if (hold /= 1) then
            do g = 1, hold-1
               do f = 1,maxlength
...
15          if (control ==1) then
               ...
               if(x<= t1 .AND. z1 == 1) then
                  i = modulo(i,gridsizex)+1
                  z1=0
               else if (x> t1 .AND. x<=t2 .AND. z2 == 1) then
                  i = modulo(i - 2,gridsizex)+1
                  z2=0
...
!do l = 1,maxl-1,1
!	if(bdi(m,l)==0) write(6,*) 'fail in setup',m,l
!end do
    ...
    integer::m,l,no1,no2,maxl,z,a
        double precision :: vb,vc,dummy1,dummy2
    character(len = 10) :: BIN
...
read(108,*) sumclient(a,:),dummy1,dummy2,unbound(a,:),&
        totalbindevents(1),totalbindevents(2),totalbindevents(3),locb(1),locb(2),bindinghist(:)
sumsep = dummy1*sumclient(a,clientlength+1)
bulksumsep = dummy2*sumclient(a,2*(clientlength+1))  
!read(108,*)sumclient(a,:),vc,vb,unbound(a,:),totalbindevents(1),totalbindevents(2),totalbindevents(3)
    end do
close(108)


open(1003,file='z.dat',action='read')
        read(1003,*) zloc(:)
close(1003)

 open(1118,file = 'location.dat',action = 'read')
read(1118,*) loc(1),loc(2)
close(1118)
...
!write(6,*) 'pickmoves complete'

!do m = 1,nprotein
!maxl = chlen(m)
!do l = 1,maxl-1
!if(bdi(m,l) == 0) then
!write(6,*) 'BDI FAIL',m,l,decide
!stop 13
!end if
!end do
!end do
...
!write(6,*) 'PIVOTS',pivotx,pivoty,pivotz
    ...
    if (pivotx .eqv. .true.) then
       ...
       do s= l+1,p-1,1
          ...
          if (dx1(s) > gridsizex/2) dx1(s) = dx1(s) - gridsizex
          if (dx1(s) < -gridsizex/2) dx1(s) = dx1(s) + gridsizex
...
!write(6,*) 'fun'
          do s=l,p,1
             ...
             !write(6,*) 'rbond',m,s,tbo(s),l,p
          end do
!write(6,*) 'rbond complete'
          do s = l,p-1,1
 !           write(6,*) 'ribond start',m,s,tbdi
		 call ribond(m,s,tbdi,3,1)
!		write(6,*) 'ribond finish'	
          end do
...
do str = l+1,p-1,1
if(tempcoord(str)%x<gridsize+1) then
       ...
       do str = st+3,maxl,1
          ...
          if((adjver.eqv. .true.)) then
             call bondformintra(m,st,str,tempcoord,tbo,bdir,deltaenergy)
                ...
                if((tempcoord(st)%x < gridsize) .or. (tempcoord(st)%x>(gridsizex-1))) then
if(cell(modulo(tempcoord(st)%x,gridsizex)+1,tempcoord(st)%y,tempcoord(st)%z)%bm/=0) then
                pr=cell(modulo(tempcoord(st)%x,gridsizex)+1,tempcoord(st)%y,tempcoord(st)%z)%bm
                str=cell(modulo(tempcoord(st)%x,gridsizex)+1,tempcoord(st)%y,tempcoord(st)%z)%bl
        bdir = -1
                      call bondforminter(m,pr,st,str,tempcoord,tbo,bdir,deltaenergy)
end if
end if

        if((tempcoord(st)%x > 1).and.(tempcoord(st)%x < gridsize+2)) then
        if(cell(modulo(tempcoord(st)%x-2,gridsizex)+1,tempcoord(st)%y,tempcoord(st)%z)%bm/=0) then
                pr=cell(modulo(tempcoord(st)%x-2,gridsizex)+1,tempcoord(st)%y,tempcoord(st)%z)%bm
                str=cell(modulo(tempcoord(st)%x-2,gridsizex)+1,tempcoord(st)%y,tempcoord(st)%z)%bl
        bdir = 1
                      call bondforminter(m,pr,st,str,tempcoord,tbo,bdir,deltaenergy)
end if
end if
...
if(tempcoord(st)%x<gridsize+1) then
...
maxl = chlen(m)
    ...
    if ((abs(mod(protcoords(m,l+1)%x - protcoords(m,l-1)%x,gridsizex)) /= 2.0) &
         .and. (abs(mod(protcoords(m,l+1)%y - protcoords(m,l-1)%y,gridsizex)) /= 2.0) &
    ...
    if (rac .eqv. .true.) then    
       ...
       if(deltax1 == gridsizex-1) deltax1 = -1
       if(deltax1 == 1-gridsizex) deltax1 = 1
       deltax2 =protcoords(m,l-1)%x - protcoords(m,l)%x
       if(deltax2 == gridsizex-1) deltax2 = -1
       if(deltax2 == 1-gridsizex) deltax2 = 1
...
if(time == 63385) write(6,*) 'params of pivot',m,l
    if(l > maxl/2) then
       ...
       do g = l+1,maxl,1
          ...
          xhold = delx(g)
          if(delx(g) > gridsizex/2) delx(g) =  xhold-gridsizex
          if(delx(g) < -gridsizex/2) delx(g) = gridsizex + xhold
...
if(tempcoord(st)%x<gridsize+1) then
       ...
       do g = 1,l-1,1
          ...
          xhold = delx(g)
          if(delx(g) > gridsizex/2) delx(g) =  xhold-gridsizex
          if(delx(g) < -1.0*gridsizex/2) delx(g) = gridsizex + xhold
          dely(g) = protcoords(m,g)%y - protcoords(m,l)%y
          yhold = dely(g)
          if(dely(g) > gridsize/2) dely(g) = yhold - gridsize
...
reptpreenforward = reptpreenforward + 1
...
!if(finalfail .eqv. .true.) then
!write(6,*) tempcoord(maxl)%x,tempcoord(maxl)%y,tempcoord(maxl)%z,bonddd(m,maxl),red(1:3),protcoords(m,1)%x
!end if       
...
83  if (reptcont .eqv. .false.) then
  ...
  subroutine reptationadjust(m,beadmin,beadmax,tempcoord,del)
    ...
    do l =beadmin,beadmax,del
       ...
       if(g /=0) then
          if(g == m) then
             !if((del == 1) .and. (protcoords(m,l+del)%dl > l +2)) then
             !protcoords(m,l)%dl = protcoords(m,l+del)%dl - del
             !else if((del == -1) .and. (protcoords(m,l+del)%dl < l-2))then
             !protcoords(m,l)%dl = protcoords(m,l+del)%dl - del
             !else
       ...
       if(g /=0) then
          if(g == m) then
             !if((del ==1) .and. (protcoords(m,l+del)%el > l+2)) then
             !protcoords(m,l)%el = protcoords(m,l+del)%el - del
             !else if((del ==-1) .and. (protcoords(m,l+del)%el < l-2)) then
             !protcoords(m,l)%el = protcoords(m,l+del)%el - del
             !else
       ...
       if(g /=0) then
          if(g == m) then
             !if((del == 1) .and. (protcoords(m,l+del)%fl > l+2)) then
             !protcoords(m,l)%fl = protcoords(m,l+del)%fl - del
             !else if((del == -1) .and. (protcoords(m,l+del)%fl < l-2)) then
             !  protcoords(m,l)%fl = protcoords(m,l+del)%fl - del
             !else
...
37  if(protcoords(m,l)%fm ==m) then
  ...
  subroutine updateposold(chainnum,beadmin,beadmax,tempcoord,tbo,maxl)
  ...
  subroutine updatepos(chainnum,beadmin,beadmax,tempcoord,tbo,tbdi,maxl)
	...
	!if (tbdi(beadnum) == 0 ) write(6,*) 'tbdi FAILLLL',beadnum
    end do
  ...
  subroutine bonddirection2(tempbonddd)
...
!linkerlength = 7
  ...
  subroutine rms(m)
    ...
    msdx = (min(abs(com(1,m)%x - initcom(m)%x), &
         (gridsize - abs(com(1,m)%x -initcom(m)%x))))**2
    msdy = (min(abs(com(1,m)%y -initcom(m)%y), &
         (gridsize - abs(com(1,m)%y -initcom(m)%y))))**2
    msdz = (min(abs(com(1,m)%z - initcom(m)%z), &
         (gridsize - abs(com(1,m)%z -initcom(m)%z))))**2
  ...
  subroutine dataout
    ...
    do m = 1,nprotein,1
       ...
       do l = 1,maxl,1
          ...
          if(bonddd(m,l) == 1) bondposit(m,l)%x = modulo(bondposit(m,l)%x + 0.5,real(gridsizex))
          if(bonddd(m,l) == -1) bondposit(m,l)%x = modulo(bondposit(m,l)%x - 0.5,real(gridsizex))
...
if(m>nscaffold) then
 dx = min(abs(protcoords(m,l)%x - protcoords(m,l-1)%x),gridsizex-abs(protcoords(m,l)%x &
             ...
             dz = min(abs(protcoords(m,l)%z - protcoords(m,l-1)%z),gridsize-abs(protcoords(m,l)%z &
              ...
              sumdebug = sqrt(real((dx**2) + (dy**2) + (dz**2)))
             if(sumdebug > protcoords(m,1)%linker) then
                write(6,*) 'SPLIT2',m,l,l-1,sumdebug
                finalfail = .true.
             end if
          end if
end if
          ...
          do a = 1,clsize-1,1
             ...
             do g = a+1,clsize
                ...
                if(cb(b,f)==1) then
                   ...
                   if (delx > (gridsizex)/2) delx = delx-gridsize
...
!allocate(client(nspecies,maxlengthstart))
	...
	vtype2= vtype2+1
       ...
       CASE ('LATTICE_DIMENSIONS')
          ...
          !allocate(interen(vtype2,vtype2))
          !allocate(interenergy(vtype2,vtype2))
	!allocate(intraenergy(vtype2,vtype2))
