...
contains
 ...
 subroutine vectorspin(m,l,maxl)
    ...
    integer::p,f,dum1,dum2,no1,no2,maxt,g,bdir,maxback,pr,str,Np,Nc
...
if(protcoords(m,l)%fm /=0) then
 ...
 !write(6,*) 'aaaaaaaaaaaaaa'

    if(Energydecision(deltaenergy,Np,Nc) .eqv. .true.) then
...
17     maxl = chlen(m)
  ...
  logical Function Energydecision(denergy,Np,Nc)
    double precision, intent(in) :: denergy
    integer,intent(in)::Np,Nc
    if(denergy <= 0.0) then
       Energydecision = .True.
    else if((Np/Nc)*exp(-denergy/kT) > ran2(seed)) then
       Energydecision = .True.
    else
  ...
  subroutine crankshaftmove
    ...
    integer :: dummy,dummy2,p,s,m,l,str,st,pr,sign,probs,maxl,maxback,bdir,choose3,Np,Nc
...
37  if(protcoords(m,l)%fm ==m) then
  ...
  subroutine rightanglemove
    ...
    integer:: st,pr,maxl,maxback,Np,Nc
    type(prottemp),dimension(:),allocatable :: tempcoord
    integer,dimension(:),allocatable::tbo,tbdi
    allocate(tempcoord(maxlength))
    allocate(tbo(maxlength))
    ...
    if(protcoords(m,l)%am > 0) Nc = Nc+1
    if(protcoords(m,l)%bm > 0) Nc = Nc+1
    if(protcoords(m,l)%cm > 0) Nc = Nc+1
    if(protcoords(m,l)%dm > 0) Nc = Nc+1
    if(protcoords(m,l)%em > 0) Nc = Nc+1
    if(protcoords(m,l)%fm > 0) Nc = Nc+1

    if(Nc>0) then
       
       if(tempcoord(l)%am > 0) Np = Np+1
       if(tempcoord(l)%bm > 0) Np = Np+1
       if(tempcoord(l)%cm > 0) Np = Np+1
       if(tempcoord(l)%dm > 0) Np = Np+1
       if(tempcoord(l)%em > 0) Np = Np+1
       if(tempcoord(l)%fm > 0) Np = Np+1
    else if(Nc == 0) then
       Np = 1
       Nc = 1
    end if
...
51  if(racc .eqv. .true.) continue
  ...
  subroutine pivot(m,l,maxl)
    ...
    integer :: b,g,xhold,yhold,zhold,str,st,pr,choose3,maxback,bdir,base,base2,Np,Nc
...
31  continue
  ...
  subroutine reptation
    ...
    integer:: st,pr,dx,dy,dz,chain2,beads2,bdir,maxl,maxback,str,Np,Nc
...
moved = 6
    ...
    do rvv = 1,clsize
       vv = cllist(rvv)
          ...
          call clustermove1(vv,dx,dy,dz,tempcluscoord,moveallow,ctbo,ctbdi,cllist,cl,moved)
       ...
       tcom(vv)%z = modulo(com(1,vv)%z + dz -1,real(gridsize))+1
        ...
        do rvv = 1,clsize
       vv = cllist(rvv)
          call clustermove2(vv,dx,dy,dz,tempcluscoord,moveallow,ctbo,ctbdi,cllist,cl)
       if(moveallow .eqv. .false.) return
       tcom(vv)%x = modulo(com(1,vv)%x + dx -1,real(gridsize))+1
       tcom(vv)%y = modulo(com(1,vv)%y + dy -1,real(gridsize))+1
       tcom(vv)%z = modulo(com(1,vv)%z + dz -1,real(gridsize))+1
...
31  if(clusmove .eqv. .false.) moveallow = .false.
  ...
  subroutine clustermove1(chainno,delx,dely,delz,tempcluscoord,moveallow,ctbo,ctbdi,cllist,cl,moved)
...
31  if(clusmove .eqv. .false.) moveallow = .false.
    ...
    subroutine clustermove2(chainno,delx,dely,delz,tempcluscoord,moveallow,ctbo,ctbdi,cllist,cl)
    integer,intent(in) :: chainno,delx,dely,delz
    integer,dimension(:,:),intent(inout)::ctbo,ctbdi
    logical,dimension(:),intent(in)::cl
    logical,intent(inout)::moveallow
