program move
  ...
  integer,dimension(:,:),allocatable ::study,sumclient,visit,visit2,unbound,bdi
  ...
  integer::count_rate,count_max,start,finish,midpoint,vtype2
...
!integer,dimension(:,:),allocatable::supercala
!integer,dimension(6)::remove
integer::reptattback,reptattforward,reptmoveback,reptmoveforward
integer::tratt,tracc,trrej,cracc,crrej,rarej,raacc,pvrej,pvacc
character(len=40)::pathtofile
integer::avesepcount,surfacecount
...
suffclust = .false.
  ...
  totdire = 0.0
  totiso = 0.0d0
  do ft = 1,vtype2!-nclientspecies
     do gt = 1,vtype2!-nclientspecies
...
open(161,file='dimens.dat',action='write')
  ...
  allocate(bdi(nprotein,maxlength))
  allocate(vdist(4*clientlength,gridsize+gridsizex))
allocate(clusdist(2*mtype,gridsize+gridsizex))
allocate(loc(2))
allocate(cell(gridsizex,gridsize,gridsize))
allocate(bindinghist(clientlength+1))
allocate(zloc(gridsizex+gridsize))
...
call initialisearray
  ...
  settingup = .true.
             dpcomclusterinit%x = real(gridsizex+gridsize)/2
...
if(modulo(time,100)==0) then
     ...
     if (fail .eqv. .true.) then
        ...
        write(6,*) 'step =',time
    write(9999,*) time,loc(:),locb(:),avesep/avesepcount,aveete/(avesepcount/9)
	 end if
	...
	write(19,*) 'translation',tracc,tratt,real(tracc)/tratt
...
real(bonddeleteacccount)/(bonddeleterejcount+bonddeleteacccount)
     ...
     if((real(modulo(midpoint-start,count_max))/count_rate)> 234000) then
        backtime = time
        goto 93
     end if
...
do ft = 1,nscaffold
do qt = 1,chlen(ft)
if(protcoords(ft,qt)%x > gridsizex) dright = dright+1
  ...
  subroutine cleanup
    ...
    do a = 1,nclients
       write(108,*) sumclient(a,:),sumsep/sumclient(a,clientlength+1),bulksumsep/sumclient(a,2*(clientlength+1)),unbound(a,:),&
        totalbindevents(:),locb(1),locb(2),surfacecount,bindinghist(:)
    end do
...
15  continue
    ...
    tempcoord(l)%x = modulo(protcoords(m,l)%x+dx-1,gridsizex+gridsize)+1
    ...
    tempcoord(l)%z = modulo(protcoords(m,l)%z+dz-1,gridsize)+1
         ...
         tempcoord(l)%x = int(ran2(seed)*gridsizex+gridsize)+1
...
if(tempcoord(l)%x<(gridsizex+1))then
...
goto 37
  ...
  !  end do
                ...
                if((tempcoord(l)%x < gridsizex).or.(tempcoord(l)%x>((gridsizex+gridsize)-1))) then
if(cell(modulo(tempcoord(l)%x,(gridsize+gridsizex))+1,tempcoord(l)%y,tempcoord(l)%z)%bm/=0)then
                pr=cell(modulo(tempcoord(l)%x,(gridsizex+gridsize))+1,tempcoord(l)%y,tempcoord(l)%z)%bm
                str=cell(modulo(tempcoord(l)%x,(gridsizex+gridsize))+1,tempcoord(l)%y,tempcoord(l)%z)%bl
        bdir = -1
                      call bondforminter(m,pr,l,str,tempcoord,bdir,deltaenergy)
end if
end if

                if((tempcoord(l)%x > 1).and.(tempcoord(l)%x < gridsizex+2)) then
if(cell(modulo(tempcoord(l)%x-2,(gridsizex+gridsize))+1,tempcoord(l)%y,tempcoord(l)%z)%bm/=0)then
...
37  if (rac .eqv. .false.) then
   ...
   subroutine translation
    ...
    double precision :: deltaenergy,direction
    logical :: rac,racc,adjver
    integer:: st,pr,maxl,maxback
    type(prottemp),dimension(:),allocatable :: tempcoord
    integer::gate,control,moved
    integer,dimension(:),allocatable::tbdi
...
do f=1,maxl,1
if(abs(f-l)>2) then
call adjacentallmove(m,l,f,tempcoord,adjver,bdir)
if(adjver .eqv. .true.) then
call bondformintra(m,l,f,tempcoord,bdir,deltaenergy)
end if
end if
...
if(tempcoord(l)%x<(gridsizex+1)) then
    ...
    if(Energydecision(deltaenergy) .eqv. .true.) then
       totalenergy = totalenergy + deltaenergy
       call updatepos(m,1,maxl,tempcoord,tbdi,maxl)
...
!phasecountdoub = phasecountdoub+1
       ...
       do l = 1,maxl
          ...
          dx = min(abs(protcoords(m,l)%x - dpcomclusterinit%x), (gridsizex+gridsize) - abs(protcoords(m,l)%x - dpcomclusterinit%x))
...
if(left .eqv. .false.) then
       ...
       maxl = chlen(m)
          dx = min(abs(protcoords(m,l)%x - (gridsize+(gridsize/2.0))), (gridsizex+gridsize) - &
(abs(protcoords(m,l)%x - ((gridsizex+(gridsize/2.0))))))
   ...
   vdist((2*clientlength)+1,lx+1) = vdist((2*clientlength)+1,lx+1) + 1.0
       ...
       maxl = chlen(m)
          dx = min(abs(protcoords(m,l)%x - (gridsizex/2.0)), (gridsizex+gridsize) - abs(protcoords(m,l)%x - ((gridsizex/2.0))))
          dy = min(abs(protcoords(m,l)%y - dpcomclusterinit%y), gridsize - abs(protcoords(m,l)%y - (gridsize/2.0)))
          dz = min(abs(protcoords(m,l)%z - dpcomclusterinit%z), gridsize - abs(protcoords(m,l)%z - (gridsize/2.0)))
...
call comfind(m,.false.)
  if(com(1,m)%x >=(gridsizex+1.0)) then
     loc(2) = loc(2)+1
	lc = .false.
  else if(com(1,m)%x <(gridsizex+1.0)) then
     loc(1) = loc(1)+1
	lc = .true.
     end if
...
if(study(a,g) ==0) unbound(a,g) =unbound(a,g)+1
 ...
 !call comlargecomp(clsize,dummycom,checklist,route,rcounter,comx,comy,comz,toolarge)
    ...
    do m = 1,nprotein
       maxl = chlen(m)
       do l = 1,maxl
          sumsq%x = min(abs(protcoords(m,l)%x-comcluster%x),(gridsizex+gridsize)-abs(protcoords(m,l)%x-comcluster%x))
    ...
    do g = 1,clsize
       ...
       do l = 1,maxl
          ...
          if(((protcoords(m,l)%x-comcluster%x)>maxx) .and. &
               ((protcoords(m,l)%x-comcluster%x)<((gridsizex+gridsize)/2))) maxx =(protcoords(m,l)%x-comcluster%x)
          if(((((gridsizex+gridsize)+protcoords(m,l)%x)-comcluster%x)>maxx).and. &
               ((((gridsizex+gridsize)+protcoords(m,l)%x)-comcluster%x)<((gridsizex+gridsize)/2))) then
          maxx =(((gridsizex+gridsize)+protcoords(m,l)%x)-comcluster%x)
       end if
       if(((comcluster%x-protcoords(m,l)%x)>minx) .and. &
           ((comcluster%x-protcoords(m,l)%x)<((gridsizex+gridsize)/2))) minx =(comcluster%x-protcoords(m,l)%x)
          if(((((gridsizex+gridsize)+comcluster%x)-protcoords(m,l)%x)>minx).and. &
               ((((gridsizex+gridsize)+comcluster%x)-protcoords(m,l)%x)<((gridsizex+gridsize)/2))) then
          minx =(((gridsizex+gridsize)+comcluster%x)-protcoords(m,l)%x)
       end if       
...
if(l==44) protcoords(m,l)%type=3
if(l==45)protcoords(m,l)%type=3
if(l==46)protcoords(m,l)%type=2
...
protcoords(m,1)%linker = specieslinker(protcoords(m,1)%type)
  ...
  subroutine setupclient(m)
   ...
   integer :: z1,z2,z3,z4,z5,z6,t1,t2,t3,t4,t5,t6 
   double precision::x
maxl = chlen(m)
...
35    continue
         l = 1
         i = int(ran2(seed)*(gridsizex+gridsize))+1
         j = int(ran2(seed)*gridsize)+1
         k = int(ran2(seed)*gridsize)+1
         !write(6,*) 'e'
         if (hold /= 1) then
            do g = 1, hold-1,1
        !write(6,*) 'gx',g,protcoords(g,1)%species,hold,m     
...
15          if (control ==1) then
               ...
               if(x<= t1 .AND. z1 == 1) then
                  i = modulo(i,gridsize+gridsizex)+1
                  z1=0
               else if (x> t1 .AND. x<=t2 .AND. z2 == 1) then
                  i = modulo(i - 2,gridsizex+gridsize)+1
                  z2=0
       ...
       if((protcoords(m,l+1)%x - protcoords(m,l)%x == 1) .or. (protcoords(m,l+1)%x - protcoords(m,l)%x ==&
            -((gridsizex+gridsize)-1))) bdi(m,l) = 1
       if((protcoords(m,l+1)%x - protcoords(m,l)%x == -1) .or. (protcoords(m,l+1)%x - protcoords(m,l)%x ==&
            ((gridsizex+gridsize)-1))) bdi(m,l) = -1
...
open(108,file= 'bonddate.dat',action= 'read')
    do a = 1,nclients
       read(108,*)sumclient(a,:),vc,vb,unbound(a,:),totalbindevents(:),locb(1),locb(2),&
bindinghist(:)
    end do
close(108)
...
do m=(nprotein-nclients)+1,nprotein,1
    ...
    double precision::pivlen
    real::bondformdelete
scans = 1


    decide = int(ran2(seed)*14)+1  !14
!write(6,*) 'decide',decide
    ...
    else if((decide > 4) .and. (decide <= 6)) then
 call reptation
   else if((decide > 6) .and. (decide <= 8)) then
call crankshaftmove
   else if((decide > 8) .and. (decide <= 10)) then
	       m = ((nprotein-nclients) +int(ran2(seed)*nclients))+1
		maxl = chlen(m)
		 pivlen = int(ran2(seed)*(maxl/2))+1
       if(pivlen >= gridsize/2) return
       ...
       if(ran2(seed) -0.5 > 0) then
		...
		call pivot(m,l,maxl)
    else if((decide > 10) .and. (decide <= 12)) then
call rightanglemove
    else if((decide > 12) .and. (decide <= 14)) then
tratt = tratt + 1
call translation
end if
...
if(maxl ==1) then
    ...
    if ((abs(mod(protcoords(m,l+1)%x - protcoords(m,l-1)%x,(gridsizex+gridsize))) /= 2.0) &
         ...
         .and. (abs(mod(protcoords(m,l+1)%x - protcoords(m,l-1)%x,(gridsizex+gridsize))) /= (gridsizex+gridsize)-2.0) &
    ...
    if (rac .eqv. .true.) then    
       ...
       if(deltax1 == (gridsizex+gridsize)-1) deltax1 = -1
       if(deltax1 == 1-(gridsizex+gridsize)) deltax1 = 1
       deltax2 =protcoords(m,l-1)%x - protcoords(m,l)%x
       if(deltax2 == (gridsizex+gridsize)-1) deltax2 = -1
       if(deltax2 == 1-(gridsizex+gridsize)) deltax2 = 1
   ...
   !if(fakey /= tempcoord(l)%y) write(6,*) 'y change'
    !if(fakez /= tempcoord(l)%z) write(6,*) 'z change'
       ...
       if(Energydecision(deltaenergy) .eqv. .true.) then
          ...
          totalenergy = totalenergy + deltaenergy
          call updatepos(m,l-1,l+1,tempcoord,tbdi,maxl)
          call updatebond(m,l,l,tempcoord)
...
51  if(racc .eqv. .true.) continue
  ...
  subroutine rotate(m,l,b,tempcoords,xx,xy,xz,yx,yy,yz,zx,zy,zz,dx,dy,dz)
    ...
    !write(6,*) dx,dy,dz
    tempcoords(b)%x = modulo(protcoords(m,l)%x + (xx*dx) + (xy*dy) + (xz*dz) -1,gridsizex+gridsize)+1
   ...
   subroutine crankshaftmove
    ...
    double precision :: deltaenergy
    integer :: dummy,dummy2,p,s,m,l,str,st,pr,sign2,probs,maxl,maxback,bdir,choose3
...
!write(6,*) 'crank start'
    ...
    do s=l+1,p-1,1
if(protcoords(m,s)%a111>0) then
       crankcont = .false.
        goto 43
     end if
     end do
    
!write(6,*) 'probability',probs
...
!write(6,*) 'PIVOTS',pivotx,pivoty,pivotz
    crankcont = .true.
    ...
    if (pivotx .eqv. .true.) then
       ...
       do s= l+1,p-1,1
          ...
          if (dx1(s) > (gridsizex+gridsize)/2) dx1(s) = dx1(s) -(gridsizex+gridsize)
          if (dx1(s) < -(gridsizex+gridsize)/2) dx1(s) = dx1(s) +(gridsizex+gridsize)
...
do str = l+1,p-1,1
if(tempcoord(str)%x<(gridsizex+1)) then
if(cell(tempcoord(str)%x,tempcoord(str)%y,tempcoord(str)%z)%bm /=0) then
crankcont = .false.
goto 43
end if
end if
...
deltaenergy = 0.0d0        
          ...
          call removeenergyintracrank(m,st,deltaenergy,tempcoord,l+1,p-1,choose3)
         if(st>3) then 
          do str = 1,st-3,1
             call adjacent(m,st,str,tempcoord,adjver,bdir)
             if((adjver.eqv. .true.)) then
                call bondformintra(m,st,str,tempcoord,bdir,deltaenergy)
             end if
          end do
end if
if(st<(maxl-2)) then
          do str = st+3,maxl,1
             ...
             if((adjver.eqv. .true.)) then
                ...
                if((tempcoord(st)%x < gridsizex).or.(tempcoord(st)%x>((gridsizex+gridsize)-1))) then
if(cell(modulo(tempcoord(st)%x,(gridsize+gridsizex))+1,tempcoord(st)%y,tempcoord(st)%z)%bm/=0)then
                pr=cell(modulo(tempcoord(st)%x,(gridsizex+gridsize))+1,tempcoord(st)%y,tempcoord(st)%z)%bm
                str=cell(modulo(tempcoord(st)%x,(gridsizex+gridsize))+1,tempcoord(st)%y,tempcoord(st)%z)%bl
        bdir = -1
                      call bondforminter(m,pr,st,str,tempcoord,bdir,deltaenergy)
end if
end if

                if((tempcoord(st)%x > 1).and.(tempcoord(st)%x < gridsizex+2)) then
if(cell(modulo(tempcoord(st)%x-2,(gridsizex+gridsize))+1,tempcoord(st)%y,tempcoord(st)%z)%bm/=0)then
                pr=cell(modulo(tempcoord(st)%x-2,gridsize)+1,tempcoord(st)%y,tempcoord(st)%z)%bm
                str=cell(modulo(tempcoord(st)%x-2,gridsize)+1,tempcoord(st)%y,tempcoord(st)%z)%bl
        bdir = 1
                      call bondforminter(m,pr,st,str,tempcoord,bdir,deltaenergy)
end if
end if
...
if(tempcoord(st)%x<(gridsizex+1)) then
        if(cell(tempcoord(st)%x,modulo(tempcoord(st)%y,gridsize)+1,tempcoord(st)%z)%bm/=0) then
                pr=cell(tempcoord(st)%x,modulo(tempcoord(st)%y,gridsize)+1,tempcoord(st)%z)%bm
                str=cell(tempcoord(st)%x,modulo(tempcoord(st)%y,gridsize)+1,tempcoord(st)%z)%bl
        bdir = -2
                      call bondforminter(m,pr,st,str,tempcoord,bdir,deltaenergy)
end if

        if(cell(tempcoord(st)%x,modulo(tempcoord(st)%y-2,gridsize)+1,tempcoord(st)%z)%bm/=0)then
                pr=cell(tempcoord(st)%x,modulo(tempcoord(st)%y-2,gridsize)+1,tempcoord(st)%z)%bm
                str=cell(tempcoord(st)%x,modulo(tempcoord(st)%y-2,gridsize)+1,tempcoord(st)%z)%bl
        bdir = 2
                      call bondforminter(m,pr,st,str,tempcoord,bdir,deltaenergy)
end if

        if(cell(tempcoord(st)%x,tempcoord(st)%y,modulo(tempcoord(st)%z,gridsize)+1)%bm/=0)then
                pr=cell(tempcoord(st)%x,tempcoord(st)%y,modulo(tempcoord(st)%z,gridsize)+1)%bm
                str=cell(tempcoord(st)%x,tempcoord(st)%y,modulo(tempcoord(st)%z,gridsize)+1)%bl
        bdir = -3
                      call bondforminter(m,pr,st,str,tempcoord,bdir,deltaenergy)
end if


        if(cell(tempcoord(st)%x,tempcoord(st)%y,modulo(tempcoord(st)%z-2,gridsize)+1)%bm/=0)then
                pr=cell(tempcoord(st)%x,tempcoord(st)%y,modulo(tempcoord(st)%z-2,gridsize)+1)%bm
                str=cell(tempcoord(st)%x,tempcoord(st)%y,modulo(tempcoord(st)%z-2,gridsize)+1)%bl
        bdir = 3
                      call bondforminter(m,pr,st,str,tempcoord,bdir,deltaenergy)
end if
end if
...
!write(6,*) 'pre update'
    ...
    if(Energydecision(deltaenergy) .eqv. .true.) then
       ...
       totalenergy = totalenergy + deltaenergy
       call updatepos(m,l,p,tempcoord,tbdi,maxl)
       call updatebond(m,l+1,p-1,tempcoord)
       call unboundcounter

       !do s=l,p,1
        !write(6,*) 'crankinfo',s,bdi(m,s),tbdi(s)
        !end do
...
37  if(protcoords(m,l)%fm ==m) then
  ...
  subroutine ribond(m,s,tbdi,axis,rotor)
    ...
    sign2 = bdi(m,s)/abs(bdi(m,s))

    !write(6,*) 'sign2'
    if(axis == 1) then
       if(rotor == -1) then
          if(abs(bdi(m,s)) == 2) tbdi(s) = sign2*(abs(bdi(m,s)) + 1)
          if(abs(bdi(m,s)) == 3) tbdi(s) = -1*sign2*(abs(bdi(m,s)) - 1)
          return
       else if(rotor == 1) then
          if(abs(bdi(m,s)) == 2) tbdi(s) = -1*sign2*(abs(bdi(m,s)) + 1)
          if(abs(bdi(m,s)) == 3) tbdi(s) = sign2*(abs(bdi(m,s)) - 1)
          return
       end if
    else if(axis ==2 ) then
       if(rotor == -1) then
          if(abs(bdi(m,s)) == 1) tbdi(s) = -1*sign2*(abs(bdi(m,s)) + 2)
          if(abs(bdi(m,s)) == 3) tbdi(s) = sign2*(abs(bdi(m,s)) - 2)        
          return
       else if(rotor == 1) then
          if(abs(bdi(m,s)) == 1) tbdi(s) = sign2*(abs(bdi(m,s)) + 2)
          if(abs(bdi(m,s)) == 3) tbdi(s) = -1*sign2*(abs(bdi(m,s)) - 2)
          return
       end if

    else if(axis ==3) then
       if(rotor == -1) then
          if(abs(bdi(m,s)) == 1) tbdi(s) = sign2*(abs(bdi(m,s)) + 1)
          if(abs(bdi(m,s)) == 2) tbdi(s) = -1*sign2*(abs(bdi(m,s)) - 1)        
          return
       else if(rotor == 1) then
          if(abs(bdi(m,s)) == 1) tbdi(s) = -1*sign2*(abs(bdi(m,s)) + 1)
          if(abs(bdi(m,s)) == 2) tbdi(s) = sign2*(abs(bdi(m,s)) - 1)         
       end if
...
!return
    ...
    if(l > maxl/2) then
       ...
       do g = l+1,maxl,1
          ...
          xhold = delx(g)
          if(delx(g) > (gridsizex+gridsize)/2) delx(g) =xhold-(gridsizex+gridsize)
          if(delx(g) < -gridsize/2) delx(g) = (gridsizex+gridsize) + xhold
...
if(tempcoord(st)%x<(gridsizex+1)) then
       ...
       if(Energydecision(deltaenergy) .eqv. .true.) then
          totalenergy = totalenergy + deltaenergy
          call updatepos(m,l,maxl,tempcoord,tbdi,maxl)
          call updatebond(m,l+1,maxl,tempcoord)
          successful = successful + 1
          call counts(0,0,1,0)
          call unboundcounter
       else
       ...
       do g = 1,l-1,1
          ...
          xhold = delx(g)
          if(delx(g) > (gridsizex+gridsize)/2) delx(g) =xhold-(gridsizex+gridsize)
          if(delx(g) < -1.0*(gridsizex+gridsize)/2) delx(g) =(gridsizex+gridsize) + xhold
          dely(g) = protcoords(m,g)%y - protcoords(m,l)%y
          yhold = dely(g)
          if(dely(g) > gridsize/2) dely(g) = yhold - gridsize
...
if(tempcoord(st)%x<(gridsizex+1)) then
       ...
       if(Energydecision(deltaenergy) .eqv. .true.) then
          totalenergy = totalenergy + deltaenergy
          call updatepos(m,1,l-1,tempcoord,tbdi,maxl)
...
subroutine reptation
    !perform reptation
    double precision :: choose,deltaenergy,oe
...
allocate(tempcoord(maxl))
    ...
    if (choose > 0.0) then
reptattforward = reptattforward + 1
...
reptmoveforward = reptmoveforward+1

       tempcoord(1)%x = modulo(protcoords(m,1)%x + dx-1,gridsizex+gridsize)+1
...
!		if(st ==2) write(6,*) '!!!!!!!!!!!!!!!!!!!!!!!!!!!!! interest' 
            reptcont = .false.
             goto 83
          end if
       end do

if(tempcoord(1)%x<(gridsizex+1)) then
...
goto 83
       ...
       !if(deltaenergy<0.0)write(6,*) 'energy post intra=',deltaenergy
        ...
        if((tempcoord(1)%x < gridsizex).or.(tempcoord(1)%x>((gridsizex+gridsize)-1))) then
if(cell(modulo(tempcoord(1)%x,gridsizex+gridsize)+1,tempcoord(1)%y,tempcoord(1)%z)%bm/=0) then
                pr=cell(modulo(tempcoord(1)%x,gridsizex+gridsize)+1,tempcoord(1)%y,tempcoord(1)%z)%bm
                str=cell(modulo(tempcoord(1)%x,gridsizex+gridsize)+1,tempcoord(1)%y,tempcoord(1)%z)%bl
        bdir = -1
                ...
                if((tempcoord(1)%x > 1).and.(tempcoord(1)%x < gridsizex+2)) then

if(cell(modulo(tempcoord(1)%x-2,gridsizex+gridsize)+1,tempcoord(1)%y,tempcoord(1)%z)%bm/=0) then
                pr=cell(modulo(tempcoord(1)%x-2,gridsizex+gridsize)+1,tempcoord(1)%y,tempcoord(1)%z)%bm
                str=cell(modulo(tempcoord(1)%x-2,gridsizex+gridsize)+1,tempcoord(1)%y,tempcoord(1)%z)%bl
        bdir = 1
                      call bondforminter(m,pr,1,str,tempcoord,bdir,deltaenergy)
	red(2) = 1
...
reptattback = reptattback + 1
       ...
       if(reptcont .eqv. .false.) goto 83
reptmoveback = reptmoveback+1

       tempcoord(maxl)%x = modulo(protcoords(m,maxl)%x +dx-1,gridsizex+gridsize)+1
       ...
       do st = 2,maxl,1
		...
		if(st == maxl-1) write(6,*) 'MAXL-1 overlap - initial sequence incorrect',g1,g2,g3,g4,g5,g6	
		reptcont = .false.
             goto 83
          end if
       end do

if(tempcoord(maxl)%x<(gridsizex+1)) then
...
goto 83
       ...
       do st = 2,maxl-2,1
        ...
        if((tempcoord(maxl)%x < gridsizex).or.(tempcoord(maxl)%x>((gridsizex+gridsize)-1))) then
if(cell(modulo(tempcoord(maxl)%x,gridsizex+gridsize)+1,tempcoord(maxl)%y,tempcoord(maxl)%z)%bm/=0) then
                pr=cell(modulo(tempcoord(maxl)%x,gridsizex+gridsize)+1,tempcoord(maxl)%y,tempcoord(maxl)%z)%bm
                str=cell(modulo(tempcoord(maxl)%x,gridsizex+gridsize)+1,tempcoord(maxl)%y,tempcoord(maxl)%z)%bl
        bdir = -1
                ...
                if((tempcoord(maxl)%x > 1).and.(tempcoord(maxl)%x < gridsizex+2)) then

if(cell(modulo(tempcoord(maxl)%x-2,gridsizex+gridsize)+1,tempcoord(maxl)%y,tempcoord(maxl)%z)%bm/=0) then
                pr=cell(modulo(tempcoord(maxl)%x-2,gridsizex+gridsize)+1,tempcoord(maxl)%y,tempcoord(maxl)%z)%bm
                str=cell(modulo(tempcoord(maxl)%x-2,gridsizex+gridsize)+1,tempcoord(maxl)%y,tempcoord(maxl)%z)%bl
        bdir = 1
                      call bondforminter(m,pr,maxl,str,tempcoord,bdir,deltaenergy)
	red(2) = 1
...
if(tempcoord(maxl)%x<(gridsizex+1)) then
       ...
       if(Energydecision(deltaenergy) .eqv. .true.) then
          ...
          do l =1,maxl-1,1
		red(1) = protcoords(m,maxl)%x
               red(2) = protcoords(m,maxl)%y
               red(3) = protcoords(m,maxl)%z
          ...
          call updatepos(m,maxl-1,maxl,tempcoord,tbdi,maxl)
          ! call checking(4)
...
!write(6,*) 'ENERGYSUBROUTINE',totalenergy
  ...
  subroutine adjacentallmove(pr,ch1,ch2,tempory,adjver,bdir)
    integer,intent(in):: pr,ch1,ch2
    ...
    delx = tempory(ch1)%x - tempory(ch2)%x
    if((abs(delx) == 1) .or. (abs(delx) == ((gridsizex+gridsize) - 1))) then
       dx = 1
       bdir = delx
if(abs(delx) == ((gridsizex+gridsize) - 1)) bdir = (delx/((gridsizex+gridsize)-1))*(gridsize-1)
    else if(delx == 0) then
       dx = 0
    end if
    if(dx == 4) then
    ...
    dely = tempory(ch1)%y - tempory(ch2)%y
    ...
    delz = tempory(ch1)%z - tempory(ch2)%z
...
31  if(dx + dy + dz ==1) then
  ...
  end subroutine adjacentallmove
...
!linkerlength = 7
        ...
        do l = 2,maxl,1
           ...
           if (dcomx >protcoords(m,1)%linker) dcomx = dcomx-(gridsizex+gridsize)
           if (dcomx <(-protcoords(m,1)%linker)) dcomx = (gridsizex+gridsize)+dcomx
        ...
        com(1,m)%x = modulo(protcoords(m,1)%x +(real(comx)/(maxl)) -1,real(gridsizex+gridsize))+1
  ...
  subroutine tracking(m)
    ...
    double precision :: dcx,dcy,dcz
         ...
         dcx = com(1,m)%x-com(2,m)%x
          if (dcx > (gridsizex+gridsize)/2) dcx = dcx-(gridsizex+gridsize)
          if (dcx < -(gridsizex+gridsize)/2) dcx = (gridsizex+gridsize) + dcx
          trackx = trackx + dcx
...
if(dxtest==((gridsizex+gridsize)-1)) dxtest=-1
if(dxtest==-((gridsizex+gridsize)-1)) dxtest=1

dytest=protcoords(m,l)%y-protcoords(m,l+1)%y
if(dytest==(gridsize-1)) dytest=-1
if(dytest==-(gridsize-1)) dytest=1


dztest=protcoords(m,l)%z-protcoords(m,l+1)%z
if(dztest==(gridsize-1)) dztest=-1
if(dztest==-(gridsize-1)) dztest=1


          dx = min(abs(protcoords(m,l)%x - protcoords(m,l+1)%x), (gridsizex+gridsize)-abs(protcoords(m,l)%x - protcoords(m,l+1)%x))
...
sep2=sqrt(real(etex**2)+real(etey**2)+real(etez**2))
  ...
  subroutine rms(m)
    ...
    msdx = (min(abs(com(1,m)%x - com(2,m)%x), &
         ((gridsizex+gridsize) - abs(com(1,m)%x -com(2,m)%x))))**2
...
!avez = avez/maxl
       ...
       do l = 1, maxl,1
          rog = (min(modulo(protcoords(m,l)%x-com(1,m)%x,(gridsizex+gridsize)*1.0),((gridsizex+gridsize) - &
               modulo(protcoords(m,l)%x-com(1,m)%x,(gridsizex+gridsize)*1.0))))**2+ &
 ...
 noROGS = noROGS+1
  ...
  subroutine length
    ...
    do m = 1,nprotein,1
        ...
        maxl = chlen(m)
        dxsum = min(abs(protcoords(m,maxl)%x - protcoords(m,1)%x), &
(gridsizex+gridsize)-abs(protcoords(m,maxl)%x - protcoords(m,1)%x))
...
polymerl = polymerl + normchainl
  ...
  subroutine dataout
    ...
    write(choice,*) 'pbc', real(4*(gridsizex+gridsize)),real(4*gridsize),real(4*gridsize)
...
if(m<=nscaffold) then
 dx = min(abs(protcoords(m,l)%x - protcoords(m,l-1)%x),(gridsizex+gridsize)-abs(protcoords(m,l)%x &
                  - protcoords(m,l-1)%x))
...
allocate(client(nspecies,maxlengthstart))
	...
	vtype2= vtype2+1
          ...
          write(6,*) 'species', client(vtype,1)%species
          call get_integer(client(vtype,1)%linker)
                client(vtype,1)%linker=1   
          do f = 1,specieslen(vtype)
             ...
             write(6,*) 'client details',client(vtype,f)%type
                   if(client(vtype,f)%type>vtype2) vtype2 = client(vtype,f)%type
          end do
          nclients = nclients+speciespop(vtype)

          write(6,*) 'runtype',vtype,vtype2,specieslen(vtype),speciespop(vtype)
       ...
       CASE ('LATTICE_DIMENSIONS')
        ...
        CALL get_integer(gridsizex)
          ...
          allocate(interen(vtype2,vtype2)) !-nclientspecies,mtype-nclientspecies))
          allocate(interenergy(vtype2,vtype2))!-nclientspecies,mtype-nclientspecies))
          allocate(intraenergy(vtype2,vtype2))!-nclientspecies,mtype-nclientspecies))
          !allocate(tttt())
