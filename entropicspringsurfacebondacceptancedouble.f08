...
if(modulo(time,100)==0) then
     ...
     if (fail .eqv. .true.) then
        ...
        write(6,*) 'step =',time
    write(9999,*) time,loc(:),locb(:),avesep/avesepcount
	 end if
...
Na = protcoords(m,1)%linker
 entropycalc = -1*((3*(separ**2)*kT)/(2*(Na**2)))
    ...
    subroutine entropytele(deltaenergy,tempcoord,m,l)
    double precision,intent(inout) ::deltaenergy
    integer,intent(in)::m,l
    Type(prottemp),dimension(:),intent(in) :: tempcoord
    double precision::oldent,newent,sep1,sep2,dx,dy,dz
    ...
    newent = 0.0d0
          ...
          dx = min(abs(tempcoord(l)%x - tempcoord(l-1)%x), gridsize-abs(tempcoord(l)%x - tempcoord(l-1)%x))
          dy = min(abs(tempcoord(l)%y - tempcoord(l-1)%y), gridsize-abs(tempcoord(l)%y - tempcoord(l-1)%y))
          dz = min(abs(tempcoord(l)%z - tempcoord(l-1)%z), gridsize-abs(tempcoord(l)%z - tempcoord(l-1)%z))
...
if(cell(modulo(tempcoord(l)%x-2,gridsize)+1,tempcoord(l)%y,tempcoord(l)%z)%bm/=0) then
 ...
 do l=2,maxl
call entropytele(deltaenergy,tempcoord,m,l)
 end do
...
END SELECT
    ...
    if((time > 0) .and. (nint(initialenergy-totalenergy)/=0)) then
