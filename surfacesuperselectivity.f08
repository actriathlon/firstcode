...
suffclust = .false.
  ...
  if(restart .eqv. .false.) then
     ...
     if(noinfo .eqv. .true.) then
        ...
        if(scalinginfo .eqv. .true.) then
           ...
           open(1478,file='converbind.dat',action='write')
     ...
     if(noinfo .eqv. .true.) then
        ...
        if(scalinginfo .eqv. .true.) then
           ...
           open(1478,file='converbind.dat',access='append')
        else if(scalinginfo .eqv. .false.) then
           open(29, file = 'rms.dat', access = 'append')
           open(93, file = 'energy.dat', access = 'append')
           open(82,file = 'newclusterdata.dat', access = 'append')
  ...
  totdire = 0.0
  totiso = 0.0d0
  do ft = 1,mtype !mtype-nclientspecies
     do gt = 1,mtype !mtype-nclientspecies
...
if(modulo(time,100)==0) then
     ...
     if((modulo(time,maxtime/10) == 0) .and.(noinfo .eqv. .false.)) then
        ...
        if(totalbindevents(2) ==0) then
                write(1478,*)time,totalbindevents(1),totalbindevents(2),0.0
        else if (totalbindevents(2) > 0) then
                write(1478,*)time,totalbindevents(1),totalbindevents(2),real(totalbindevents(1))/totalbindevents(2)
                end if
     end if
...
!if(gridsize /= 30) write(6,*) 'GRIDSIZE HAS CHANGED'
...
if(left .eqv. .false.) then
       ...
       maxl = chlen(m)
          dx = min(abs(protcoords(m,l)%x - dpcomclusterinit%x), gridsize - abs(protcoords(m,l)%x - (gridsize/2.0)))
...
if(protcoords(m,1)%type==1) then
        call bonddirectionsurf(tbo(l))
else if (protcoords(m,1)%type==2) then
  call bonddirection(tbo(l))
end if
...
protcoords(m,1)%linker = specieslinker(protcoords(m,1)%type)
        protcoords(m,1)%x = rx/4.0
        protcoords(m,1)%y = ry/4.0
        protcoords(m,1)%z = rz/4.0
        ...
        if(bonddd(m,1) ==0) write(6,*) 'no bond allocation',m,1,rx,ry,rz,bondx,bondy,bondz
        do l = 2,maxl,1
...
if (tempcoord(l)%z > gridsize) goto 37 
if (tempcoord(l)%z<1) goto 37
if(l>1) then
...
if(g/=m) then
  ...
  subroutine bonddirection(tempbonddd)
   ...
   subroutine bonddirectionsurf(tempbonddd)
    integer,intent(inout) :: tempbonddd
    integer :: xp,xn,yp,yn,zp,zn,chdir
  ...
  end subroutine bonddirectionsurf
