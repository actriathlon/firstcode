  ...
  subroutine cleanup
    ...
    do a = 1,nclients
       write(108,*) sumclient(a,:),sumsep/sumclient(a,clientlength+1),bulksumsep/sumclient(a,2*(clientlength+1)),unbound(a,:),&
        totalbindevents(1),totalbindevents(2),locb(1),locb(2),surfacecount,bindinghist(:)
    end do
...
if(tempcoord(l)%z<2) return
if(tempcoord(l)%z>gridsize) return
      end do
...
if((protcoords(m,l)%z==2) .or. (protcoords(m,l)%z==gridsize)) atsurface=.true.
!if(protcoords(m,l)%z==1) write(6,*) 'FAILLLLLLLLLLLLLL client on the bottom plane!!!!!'
...
if(atsurface .eqv. .true.) surfacecount=surfacecount+1
    ...
    m = nprotein     !((nprotein-nclients) +int(ran2(seed)*nclients))+1
...
!write(6,*) 'PIVOTS',pivotx,pivoty,pivotz
    ...
    do s=l+1,p-1,1
       if(tempcoord(s)%z>gridsize) then
                crankcont = .false.
                goto 43
        end if

        if(tempcoord(s)%z<2) then
                crankcont = .false.
                goto 43
        end if
    end do
...
if(cell(modulo(tempcoord(st)%x,gridsize)+1,tempcoord(st)%y,tempcoord(st)%z)%bm/=0) then
                pr=cell(modulo(tempcoord(st)%x,gridsize)+1,tempcoord(st)%y,tempcoord(st)%z)%bm
                str=cell(modulo(tempcoord(st)%x,gridsize)+1,tempcoord(st)%y,tempcoord(st)%z)%bl
        bdir = -1
                      call bondforminter(m,pr,st,str,tempcoord,bdir,deltaenergy)
end if

if(cell(modulo(tempcoord(st)%x-2,gridsize)+1,tempcoord(st)%y,tempcoord(st)%z)%bm/=0) then
...
!return
    ...
    do g=l+1,maxl,1
       if(tempcoord(g)%z>gridsize) then
                pivcont = .false.
                goto 75
        end if

        if(tempcoord(g)%z<2) then
                pivcont = .false.
                goto 75
        end if
    end do
...
allocate(client(nspecies,maxlengthstart))
	...
	vtype2= vtype2+1
        ...
        do f = 1,specieslen(vtype)
             ...
             write(6,*) 'client details',client(vtype,f)%type,maxlengthstart
