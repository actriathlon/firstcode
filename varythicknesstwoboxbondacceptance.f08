...
open(161,file='dimens.dat',action='write')
  ...
  allocate(vdist(4*clientlength,gridsizex+gridsize))
allocate(clusdist(2*mtype,gridsizex+gridsize))
allocate(loc(2))
...
call initialisearray
  ...
  summing(:,:) = 0
  allocate(freesites(mtype*6,(gridsizex+gridsize)*binsize))
  freesites(:,:) = 0
...
write(6,*) 'GRIDSIZE',gridsizex,gridsize
...
if(protcoords(ft,qt)%x <= gridsizex) dleft = dleft+1
end do
end do
...
goto 37
  ...
  !  end do
                ...
                if((tempcoord(l)%x < gridsizex).or.(tempcoord(l)%x>((gridsize+gridsizex)-1))) then
if(cell(modulo(tempcoord(l)%x,(gridsizex+gridsize))+1,tempcoord(l)%y,tempcoord(l)%z)%bm/=0)then
...
if(cell(modulo(tempcoord(l)%x-2,(gridsizex+gridsize))+1,tempcoord(l)%y,tempcoord(l)%z)%bm/=0)then
                pr=cell(modulo(tempcoord(l)%x-2,(gridsizex+gridsize))+1,tempcoord(l)%y,tempcoord(l)%z)%bm
                str=cell(modulo(tempcoord(l)%x-2,(gridsizex+gridsize))+1,tempcoord(l)%y,tempcoord(l)%z)%bl
        bdir = 1
                      call bondforminter(m,pr,l,str,tempcoord,bdir,deltaenergy)
end if
end if
...
if(i>gridsizex) pointless1 = pointless1 + 1
if(i<=gridsizex) pointless2 = pointless2 + 1
...
!write(6,*) 'linkerlen',protcoords(m,l)%linker
               ...
               if(sqrt(real((dx**2) + (dy**2) + (dz**2))) > protcoords(m,l-1)%linker) goto 15
                     ...
                     i = modulo(xbk+dx-1,(gridsizex+gridsize))+1
...
protcoords(m,1)%a11n=0
        ...
        protcoords(m,1)%x = (rx/4)+gridsizex
        protcoords(m,1)%y = (ry/4)
        protcoords(m,1)%z = (rz/4)
        chlen(m) = specieslen(protcoords(m,1)%type+2)
        maxl = chlen(m)
...
protcoords(m,1)%a11n=0
        ...
        protcoords(m,l)%x = (rx/4)+gridsizex
        protcoords(m,l)%y = (ry/4)
        protcoords(m,l)%z = (rz/4)
...
if(l>1) then
    dx = min(abs(tempcoord(l)%x - protcoords(m,l-1)%x),(gridsizex+gridsize)-abs(tempcoord(l)%x &
         - protcoords(m,l-1)%x)) 
...
if(cell(modulo(tempcoord(l)%x,gridsizex+gridsize)+1,tempcoord(l)%y,tempcoord(l)%z)%bm/=0) then
                pr=cell(modulo(tempcoord(l)%x,(gridsizex+gridsize))+1,tempcoord(l)%y,tempcoord(l)%z)%bm
                str=cell(modulo(tempcoord(l)%x,(gridsize+gridsizex))+1,tempcoord(l)%y,tempcoord(l)%z)%bl
        bdir = -1
                      call bondforminter(m,pr,l,str,tempcoord,bdir,deltaenergy)
end if
end if
...
19     continue
     ...
     dx = min(abs(tempcoord(1)%x - protcoords(m,2)%x),(gridsizex+gridsize)-abs(tempcoord(1)%x &
         - protcoords(m,2)%x)) 
...
15     continue
     ...
     dx = min(abs(tempcoord(maxl)%x - protcoords(m,maxl-1)%x),(gridsizex+gridsize)-abs(tempcoord(maxl)%x &
  ...
  subroutine tracking(m)
    ...
    double precision :: dcx,dcy,dcz
         ...
         dcx = com(1,m)%x-com(2,m)%x
          if (dcx > (gridsizex+gridsize)/2.0) dcx = dcx-(gridsizex+gridsize)
          if (dcx < -(gridsizex+gridsize)/2.0) dcx = (gridsizex+gridsize) + dcx
          trackx = trackx + dcx
 ...
 noROGS = noROGS+1
  ...
  subroutine length
    ...
    do m = 1,nprotein,1
        ...
        maxl = chlen(m)
        dxsum = min(abs(protcoords(m,maxl)%x - protcoords(m,1)%x),((gridsizex+gridsize)- &
abs(protcoords(m,maxl)%x - protcoords(m,1)%x)))
          ...
          do a = 1,clsize-1,1
             ...
             do g = a+1,clsize
                ...
                if(cb(b,f)==1) then
                   ...
                   if (delx > ((gridsizex+gridsize))/2.0) delx = delx-(gridsizex+gridsize)
                   if (delx < -(gridsizex+gridsize)/2.0) delx = delx +(gridsizex+gridsize)
                   comx(b,f) =  delx
                   comx(f,b) = -delx
                   dely = com(1,f)%y - com(1,b)%y
                   if (dely > real(gridsize)/2.0) dely = dely-gridsize
                   if (dely < -real(gridsize)/2.0) dely = dely + gridsize
                   comy(b,f) =  dely
                   comy(f,b) = -dely
                   delz = com(1,f)%z - com(1,b)%z
                   if (delz > real(gridsize)/2.0) delz = delz-gridsize
                   if (delz < -real(gridsize)/2.0) delz = delz + gridsize
                   comz(b,f) =  delz
                   comz(f,b) = -delz
                end if
...
76        if(clsize == 1) then
          ...
          !write(6,*) dummycom%x
          comcluster%x = INT(modulo((com(1,centralchain)%x) + (dummycom%x/totcluspop)-1,real(gridsizex+gridsize))+1)
          ...
          dpcomcluster%x = modulo((com(1,centralchain)%x) + (dummycom%x/totcluspop)-1,real(gridsizex+gridsize))+1
...
comcluster%x = int(real(gridsizex+gridsize)/2.0)
comcluster%y = int(real(gridsize)/2.0)
comcluster%z = int(real(gridsize)/2.0)
        ...
        subroutine clustercomcheck(m,clsize,comcluster,cb,cllist)
          ...
          do ra = 1,clsize
             ...
             if(a/=m) then
                ...
                if (delx > real((gridsizex+gridsize))/2.0) delx =delx-(gridsizex+gridsize)
                if (delx < -real(gridsizex+gridsize)/2.0) delx = delx + (gridsizex+gridsize)
                comx = comx + (delx*maxb)
          ...
          dummycom%x = modulo((com(1,m)%x) + (comx/totcluspop)-1,real(gridsizex+gridsize))+1
    ...
    do m= 1,nprotein
       ...
       acount = acount +1
        ...
        subroutine comlargecomp(clsize,dummycom,checklist,route,rcounter,comx,comy,comz,toolarge)
          ...
          if((maxx - minx) > real((gridsizex+gridsize))/2.0) toolarge = .true.
          if((maxy - miny) > real(gridsize)/2.0) toolarge = .true.
          if((maxz - minz) > real(gridsize)/2.0) toolarge = .true.
...
write(6,*) 'freesites COM',maxclus,content,dpcomclusterinit
    do g = 1,nprotein-nclients,1
       if(protcoords(g,1)%x<=(gridsizex)) then
maxl = chlen(g)
do l = 1,maxl,1
if(bound(g,l) == 0) then
...
else if(protcoords(g,1)%x > gridsizex) then
maxl = chlen(g)
do l = 1,maxl,1      
if(bound(g,l) == 0) then
...
!phasecountdoub = phasecountdoub+1
          ...
          normaliser = normaliser + 1
          !phasecount = phasecount+1
          dx = min(abs(protcoords(m,l)%x - (gridsizex+(gridsize/2.0))), (gridsize+gridsize) - abs(protcoords(m,l)%x - &
(gridsizex+(gridsize/2.0))))
          dy = min(abs(protcoords(m,l)%y - (gridsize/2.0)), gridsize - (abs(protcoords(m,l)%y - (gridsize/2.0))))
          dz = min(abs(protcoords(m,l)%z - (gridsize/2.0)), gridsize - (abs(protcoords(m,l)%z - (gridsize/2.0))))
