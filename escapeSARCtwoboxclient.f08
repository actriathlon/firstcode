...
if(timebase == 3) call freesitescalc
     ...
     if ((mod(time,outputrate/100) == 0) .and. (film .eqv. .true.) ) then
        call dataout
     end if
!write(6,*) 'c'
     ...
     if((real(modulo(midpoint-start,count_max))/count_rate)> 180000) then
        backtime = time
        goto 93
     end if
  ...
  subroutine cleanup
    ...
    do a = 1,nclients
       write(108,*) sumclient(a,:),sumsep/sumclient(a,clientlength+1),bulksumsep/sumclient(a,2*(clientlength+1)),unbound(a,:),&
        totalbindevents(1),totalbindevents(2),totalbindevents(3),locb(1),locb(2)
    end do
...
protcoords(m,1)%linker = specieslinker(protcoords(m,1)%type)
  ...
  subroutine setupclient(m)
   ...
   integer :: z1,z2,z3,z4,z5,z6,t1,t2,t3,t4,t5,t6,clst,clfi 
double precision :: x
maxl = chlen(m)

     call system_clock(clst,count_rate,count_max)
...
47    if (protpass ==1) then
     ...
     call system_clock(clfi,count_rate,count_max)

     if((real(modulo(clfi-clst,count_max))/count_rate)> 10800) then
        write(6,*) 'TOO DENSE TO SETUP'
        stop 55
     end if
