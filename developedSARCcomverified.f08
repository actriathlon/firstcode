program move
  ...
  double precision::bondx,bondy,bondz,rx,ry,rz,readcomx,readcomy,readcomz,realtotrog
!double precision::stdev,sumvar,sumvarcount
integer,dimension(:,:),allocatable::speciestype
  integer,dimension(:),allocatable ::chlen,speciespop,specieslen,specieslink,totunb,sumradofgy
...
integer::firstrun,ft,gt,x,halfbox,limiting
  character(len = 10) :: commandread,commandread2,commandread3,commandread4,commandread5,commandread6
  character(len=10) :: u,v,w
  real(8),  parameter :: PI_8  = 4 * atan (1.0_8)
  double precision,dimension(:,:),allocatable :: interenergy,interen,intraenergy
  logical ::isbond  
integer:: norma,normb,normc,normd,realtotrogcount,r7,rogcount
integer,dimension(:,:),allocatable :: runningclusterlist
integer,dimension(6)::totclhist,totblhist
integer,dimension(:),allocatable:: energytotals,connectivitytotals,chen
integer::sumconn,sumen,sumconncount,normen,normconn
...
close(99)
open(811,file='radhist.dat',action='write')
!open(196,file='availablesites.dat',action = 'write')
open(912,file='connhist.dat',action='write')
open(913,file='aveconn.dat',action='write')
open(97,file='averog.dat',action='write')
!write(168,*) 'FAIL'
...
isbond = .false.
 ...
 call get_command_argument(6,commandread6)
  read(commandread6, '(i1)') limiting
 !call get_command_argument(7,commandread7)
  !read(commandread7, '(i4)') averog
...
allocate(energytotals(maxlength))
allocate(connectivitytotals(maxlength))
...
rogcount = 0
  ...
  do m = 1,nprotein
     read(21,*)BIN,BIN,BIN,BIN,BIN,chains(m,1)%type,BIN,BIN !chains(m,1)%type
     read(21,*)BIN,BIN,BIN,BIN,BIN,BIN,BIN,BIN
chains(m,1)%species = chains(m,1)%type/2
chains(m,1)%type=chains(m,1)%type/2
...
chains(m,1)%linker = specieslink(chains(m,1)%type)
     ...
     do l = 2,maxl
chains(m,l)%type = speciestype(chains(m,1)%type,l)
chains(m,l)%species = chains(m,1)%species
        read(21,*)BIN,BIN,BIN,BIN,BIN,BIN,BIN,BIN !,chains(m,l)%type
        read(21,*)BIN,BIN,BIN,BIN,BIN,BIN,BIN,BIN
        count = count+1
!chains(m,l)%type=chains(m,l)%type/2

chains(m,l)%linker = specieslink(chains(m,1)%type)
...
write(6,*) 'ALLSUM',allsum(:,l)

write(168,*)r,allsum(:,l)/(timeofinterest*((r**3)-((real(l-1)/binsize)**3))*PI_8*(4.0/3.0)),&
 (allsum(1,l)+allsum(2,l))/(timeofinterest*((r**3)-((real(l-1)/binsize)**3))*PI_8*(4.0/3.0)),&
 sumall/(timeofinterest*(r**3)*PI_8*(4.0/3.0)),&
 (allsum(1,l)+allsum(2,l) + allsum(3,l))/(timeofinterest*((r**3)-((real(l-1)/binsize)**3))*PI_8*(4.0/3.0))
...
do l = 1,maxlength
write(912,*)l,float(connectivitytotals(l))/sumconncount,float(energytotals(l))/sumconncount
end do


write(6,*) 'COORDINATION AVERAGE', float(sumen)/sumconncount,'CONNECTIVITY AVERAGE', float(sumconn)/sumconncount
...
83                       continue
       ...
       initialenergy = initialenergy + interen(chains(m,l)%type,chains(g,f)%type)    
                      ...
                      if((bdir == chains(g,f)%bond) .and. (bdir == (-1*chains(m,l)%bond)) .and. &
                        ...
                        chen(m) = chen(m)+interenergy(chains(m,l)%type,chains(g,f)%type)
                        chen(g) =chen(g)+interenergy(chains(g,f)%type,chains(m,l)%type)
...
!call energy(initialenergy)
   ...
   cconn =0
    ...
    do m = 1,maxclus
       g = maxcluslist(m)
!write(6,*) int(chen(g)), chen(g)
energytotals(-1*int(chen(g))) = energytotals(-1*int(chen(g))) + 1
connectivitytotals(concount(g)) = connectivitytotals(concount(g)) + 1
       cconn = cconn + concount(g)
aveconn = aveconn + concount(g)
        sumconn = sumconn + concount(g)
       if(chains(g,1)%type ==1) acconn = acconn+concount(g)
       maxclusenergy = maxclusenergy + chen(g)
sumen = sumen + chen(g)
avecoo = avecoo + chen(g)
    end do
...
totunb(3) = totunb(3) +(c-c2)
totunb(4) = totunb(4) +(d-d2)
...
sumconncount = sumconncount + maxclus
  ...
  subroutine energy(init2)
    ...
    logical :: adjver
        double precision,intent(in)::init2
    initialenergy = 0.0d0
    !totalenergy = 0.0d0
    allocate(tempcoord(maxlength))
...
if((bdir == chains(g,f)%bond) .and. (bdir == (-1*chains(m,l)%bond)) .and. &
    ...
    if((time > 0) .and. (nint(init2) /= nint(initialenergy))) then
       write(6,*) 'energyfail',init2,initialenergy,time
       !finalfail = .true.
    end if
...
!linkerlength = 7
  ...
  subroutine radiusofgyration(maxclus,maxcluslist)
    !calculates the radius of gyration of the largest cluster
    integer::m,l,maxl,nobeads,g,lx
        integer,intent(in)::maxclus
integer,dimension(:),intent(in)::maxcluslist
    double precision :: rog,totrog,minirog,averog1,var,std!,avex,avey,avez
    totrog = 0.0d0
    nobeads = 0
var = 0.0d0
std = 0.0d0
...
minirog = 0.0d0
       do l = 1, maxl,1
          rog = (min(modulo(chains(m,l)%x-com(1,m)%x,gridsize*1.0),(gridsize- &
               modulo(chains(m,l)%x-com(1,m)%x,gridsize*1.0))))**2+ &
               (min(modulo(chains(m,l)%y-com(1,m)%y,gridsize*1.0),(gridsize- &
               modulo(chains(m,l)%y-com(1,m)%y,gridsize*1.0))))**2+ &
               (min(modulo(chains(m,l)%z-com(1,m)%z,gridsize*1.0),(gridsize- &
               modulo(chains(m,l)%z-com(1,m)%z,gridsize*1.0))))**2
          totrog = totrog + rog
        minirog = minirog+rog
        realtotrog = realtotrog+rog       
        end do
          lx = INT((sqrt(minirog/maxl))/(1.0/(4*binsize)))
            sumradofgy(lx+1) =sumradofgy(lx+1)+1.0
rogcount = rogcount +1
        nobeads = nobeads+maxl
        realtotrogcount = realtotrogcount+maxl
    end do

averog1=SQRT(totrog/(nobeads))
...
var = var + (sqrt(minirog/maxl)-averog1)**2
         end do

std = sqrt(var)/(maxclus-1)

    write(97,*) t,totrog/(nobeads),SQRT(totrog/(nobeads)),std
...
55           continue
  ...
  SUBROUTINE read_setup
    ...
    INTEGER :: err, i, j,runtype,dummy2,dummytype,xl,f,dum,firstmax
    LOGICAL :: success
