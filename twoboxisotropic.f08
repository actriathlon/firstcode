...
integer,dimension(:),allocatable::unboundarray,boundarray,bindingsitehist
...
allocate(bindinghist(6*clientlength+1))
  ...
  subroutine cleanup
    ...
    do a = 1,nclients
       write(108,*) sumclient(a,:),sumsep/sumclient(a,clientlength+1),bulksumsep/sumclient(a,2*(clientlength+1)),unbound(a,:),&
        totalbindevents(1),totalbindevents(2),totalbindevents(3),locb(1),locb(2),bindinghist(:),bindingsitehist(:)
    end do
...
!phasecount = phasecount+1
  ...
  subroutine analyse
    integer::m,l,g,f,control,dx,dy,dz,maxl,a,lx,dj,pr,str
    double precision:: separation,testenergy
    logical::left,lc
    integer,dimension(clientlength) :: attached
        integer,dimension(nscaffold) :: bindsarr
...
zloc(protcoords(m,l)%x) = zloc(protcoords(m,l)%x) + 1
        ...
        if((cell(modulo(protcoords(m,l)%x,gridsizex)+1,protcoords(m,l)%y,protcoords(m,l)%z)%bm/=0)) then
        pr =cell(modulo(protcoords(m,l)%x,gridsizex)+1,protcoords(m,l)%y,protcoords(m,l)%z)%bm
        str =cell(modulo(protcoords(m,l)%x,gridsizex)+1,protcoords(m,l)%y,protcoords(m,l)%z)%bl
           if(protcoords(pr,str)%type==1) then         
zlocb(protcoords(m,l)%x) = zlocb(protcoords(m,l)%x) + 1
        ...
        if((cell(modulo(protcoords(m,l)%x-2,gridsizex)+1,protcoords(m,l)%y,protcoords(m,l)%z)%bm/=0)) then
        pr=cell(modulo(protcoords(m,l)%x-2,gridsizex)+1,protcoords(m,l)%y,protcoords(m,l)%z)%bm 
        str=cell(modulo(protcoords(m,l)%x-2,gridsizex)+1,protcoords(m,l)%y,protcoords(m,l)%z)%bl 
           if(protcoords(pr,str)%type==1) then         
zlocb(protcoords(m,l)%x) = zlocb(protcoords(m,l)%x) + 1
                control = control+1
                goto 7000
        ...
        if((cell(protcoords(m,l)%x,modulo(protcoords(m,l)%y,gridsize)+1,protcoords(m,l)%z)%bm/=0)) then 
        pr=cell(protcoords(m,l)%x,modulo(protcoords(m,l)%y,gridsize)+1,protcoords(m,l)%z)%bm
        str=cell(protcoords(m,l)%x,modulo(protcoords(m,l)%y,gridsize)+1,protcoords(m,l)%z)%bl
...
zlocb(protcoords(m,l)%x) = zlocb(protcoords(m,l)%x) + 1
        ...
        if((cell(protcoords(m,l)%x,modulo(protcoords(m,l)%y-2,gridsize)+1,protcoords(m,l)%z)%bm/=0)) then
        pr=cell(protcoords(m,l)%x,modulo(protcoords(m,l)%y-2,gridsize)+1,protcoords(m,l)%z)%bm
        str=cell(protcoords(m,l)%x,modulo(protcoords(m,l)%y-2,gridsize)+1,protcoords(m,l)%z)%bl
        if(protcoords(pr,str)%type==1) then
zlocb(protcoords(m,l)%x) = zlocb(protcoords(m,l)%x) + 1
                control = control+1
                goto 7000
        ...
        if((cell(protcoords(m,l)%x,protcoords(m,l)%y,modulo(protcoords(m,l)%z,gridsize)+1)%bm/=0)) then 
        pr = cell(protcoords(m,l)%x,protcoords(m,l)%y,modulo(protcoords(m,l)%z,gridsize)+1)%bm
        str = cell(protcoords(m,l)%x,protcoords(m,l)%y,modulo(protcoords(m,l)%z,gridsize)+1)%bl
        if(protcoords(pr,str)%type==1) then
zlocb(protcoords(m,l)%x) = zlocb(protcoords(m,l)%x) + 1
                control = control+1
                goto 7000
        ...
        if((cell(protcoords(m,l)%x,protcoords(m,l)%y,modulo(protcoords(m,l)%z-2,gridsize)+1)%bm/=0)) then
        pr=cell(protcoords(m,l)%x,protcoords(m,l)%y,modulo(protcoords(m,l)%z-2,gridsize)+1)%bm
        str=cell(protcoords(m,l)%x,protcoords(m,l)%y,modulo(protcoords(m,l)%z-2,gridsize)+1)%bl
        if(protcoords(pr,str)%type==1) then
zlocb(protcoords(m,l)%x) = zlocb(protcoords(m,l)%x) + 1
                control = control+1
                goto 7000
...
do pr=1,nscaffold,1
if(bindsarr(pr)>0) then
bindingsitehist(bindsarr(pr))=bindingsitehist(bindsarr(pr))+1
end if
...
!        str =cell(modulo(protcoords(m,l)%x,gridsizex)+1,protcoords(m,l)%y,protcoords(m,l)%z)%bl
!           if(protcoords(pr,str)%type==1) then         
!                testenergy = testenergy+  interen(protcoords(m,l)%type,protcoords(pr,str)%type)
!        end if
!        end if
!
!        if((cell(modulo(protcoords(m,l)%x-2,gridsizex)+1,protcoords(m,l)%y,protcoords(m,l)%z)%bm/=0)) then
...
!if (int(testenergy)/=int(totalenergy)) then
!write(6,*) 'TEST ENERGY FAIL!!!!',testenergy,totalenergy,control
!stop 33
!end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!TEST!DELETE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     !bindinghist((-int(totalenergy))+1) = bindinghist((-int(totalenergy))+1) +1


if(int(totalenergy) <0) then
!left = .true.
...
else if(int(totalenergy)<0) then
...
scans = 1
    ...
    if(decide <= 4) call reptation
    if((decide > 4) .and. (decide <= 8)) call positioning
   if((decide > 8) .and. (decide <= 12)) call teleport
