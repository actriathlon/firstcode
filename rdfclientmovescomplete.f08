...
scans = 1
    ...
    if((decide > 6) .and. (decide <= 10)) call positioning
   if((decide > 6) .and. (decide <= 10)) call teleport
...
allocate(client(nspecies,maxlengthstart))
       ...
       CASE ('CLIENT')
          mtype = mtype +1
          CALL get_integer(specieslen(mtype))
          CALL get_integer(speciespop(mtype))
          clientlength = specieslen(mtype)
          client(mtype,1)%species = mtype
          call get_integer(client(mtype,1)%linker)   
          do f = 1,specieslen(mtype)
             call get_integer(client(mtype,f)%type)
             client(mtype,f)%linker = client(mtype,1)%linker
             write(6,*) 'client details',client(mtype,f)%type
                          !if(dummytype>mtype) mtype = dummytype
          end do
          nclients = nclients+speciespop(mtype)

          write(6,*) 'runtype',mtype,specieslen(mtype),speciespop(mtype)
