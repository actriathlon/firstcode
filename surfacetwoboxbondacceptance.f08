       ...
       do st = 1,maxl,1
          if(st /=l) then
             if(overlaps(m,l,st,tempcoord) .eqv. .false.) then
                !write(6,*) 'OVERLAPFAILLL POS',m,l,st,protcoords(m,l)%x,protcoords(m,l)%y&
        !,protcoords(m,l)%z,tempcoord(st)%x,tempcoord(st)%y,tempcoord(st)%z
                rac = .false.
                goto 37
             end if
          end if
    ...
    if(sumdebug > real(protcoords(m,1)%linker)) then
       ...
       do st = 1,maxl-1,1
          if(overlaps(m,1,st,tempcoord) .eqv. .false.) then
                !write(6,*) 'OVERLAPFAILLL Rept',m,st,protcoords(m,1)%x,protcoords(m,1)%y&
        !,protcoords(m,1)%z,tempcoord(st)%x,tempcoord(st)%y,tempcoord(st)%z
                reptcont = .false.
             goto 83
          end if
       end do
    ...
    if(sumdebug > real(protcoords(m,1)%linker)) then
       ...
       do st = 2,maxl,1
          if(overlaps(m,maxl,st,tempcoord) .eqv. .false.) then
                !write(6,*) 'OVERLAPFAILLL Rept 2',m,st,protcoords(m,maxl)%x,protcoords(m,maxl)%y&
        !,protcoords(m,maxl)%z,tempcoord(st)%x,tempcoord(st)%y,tempcoord(st)%z
             reptcont = .false.
             goto 83
          end if
       end do
