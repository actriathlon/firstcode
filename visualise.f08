program move
  ...
  double precision::bondx,bondy,bondz,rx,ry,rz,ma,mb,mc,md
...
integer:: norma,normb,normc,normd
  ...
  open(29, file = 'basic.xyz', action = 'write')
...
!  allocate(summing(2*mtype,((gridsize/2)*binsize)+1))
...
!allocate(realsum(2*mtype,((gridsize/2)*binsize)+1)) 
...
contains
  ...
  subroutine clustercount
    ...
    integer :: acconn,cconn,atconn,tconn,a,b,c,d
    ...
    double precision::maxclusenergy,initialenergy
character(len=10) :: ID
...
!write(6,*) 'energy',en(1),timeofinterest
       ...
       write(29,*) totpoints/2 + (6*(gridsize**2))
       write(29,*) ' '
      ...
      do m = 1,nprotein
         ...
         if(clnos(m) == content) then
            ...
            if(bound(m,l)==1) ID = 'N'
            if(bound(m,l)==0) ID = 'P'
          else if(chains(m,l)%type == 23) then
             ID = 'Ne'

            end if
               write(29,*) ID,chains(m,l)%x,chains(m,l)%y,chains(m,l)%z
            end do
         else if (clnos(m) /= content) then
            do l = 1,maxl
               if(chains(m,l)%type == 1) ID = 'H'
               if(chains(m,l)%type == 2) ID = 'He'
               write(29,*) ID,chains(m,l)%x,chains(m,l)%y,chains(m,l)%z
            end do
         end if
         ...
         do m = 1,gridsize
            do l =1,gridsize
               write(29,*) 'Al',m,l,1 !(m+l)+100,m,l,0
            end do
         end do
              do m = 1,gridsize
            do l =1,gridsize
               write(29,*) 'Al',m,l,gridsize
            end do
         end do
            do m = 1,gridsize
            do l =1,gridsize
