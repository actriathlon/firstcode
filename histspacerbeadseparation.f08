!code for bead separation of chain with spacers on it
...
isbond = .false.
  ...
  allocate(summing(6,(2*maxlink*binsize)+1))
...
do lx = 1,2*maxlink*binsize,1
        rr = lx/real(binsize)
write(49,*)rr,summing(1,lx)/totmonoc,summing(2,lx)/totdic,summing(3,lx)/totclusc,&
summing(4,lx)/(totmonoc/2.0),summing(5,lx)/(totdic/2.0),summing(6,lx)/(totclusc/2.0)
...
summing(1,sss) = summing(1,sss)+1.0
 ...
 do l = 3,maxl,2
sepx=min(abs(chains(m,l)%x-chains(m,l-2)%x),gridsize-abs(chains(m,l)%x-chains(m,l-2)%x))
sepy=min(abs(chains(m,l)%y-chains(m,l-2)%y),gridsize-abs(chains(m,l)%y-chains(m,l-2)%y))
sepz=min(abs(chains(m,l)%z-chains(m,l-2)%z),gridsize-abs(chains(m,l)%z-chains(m,l-2)%z))

totsep = sqrt((real(sepx)**2)+(real(sepy)**2) +(real(sepz)**2))
sss = int(totsep*binsize)
summing(4,sss) = summing(4,sss)+1.0
...
totsep = sqrt((real(sepx)**2)+(real(sepy)**2) +(real(sepz)**2))
sss = int(totsep*binsize)
summing(5,sss) = summing(5,sss)+1.0
...
totsep = sqrt((real(sepx)**2)+(real(sepy)**2) +(real(sepz)**2))
sss = int(totsep*binsize)
summing(6,sss) = summing(6,sss)+1.0
