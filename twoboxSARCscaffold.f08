...
integer,dimension(:,:),allocatable::speciestype
...
!if(i<=gridsize) pointless2 = pointless2 + 1
	...
	if (hold /= 1) then
            do g = 1, hold-1
               maxback = chlen(g) !specieslen(protcoords(g,1)%species)
               do f = 1,maxback
...
read(21,*)BIN,BIN,BIN,BIN,BIN,protcoords(m,1)%type,BIN,BIN !chains(m,1)%type
     ...
     do l = 2,maxl
        protcoords(m,l)%type = speciestype(protcoords(m,1)%type,l)
        read(21,*)BIN,BIN,BIN,BIN,BIN,BIN,BIN,BIN !,chains(m,l)%type
        read(21,*)BIN,BIN,BIN,BIN,BIN,BIN,BIN,BIN
        count = count+1
        !chains(m,l)%type=chains(m,l)%type/2
protcoords(m,l)%species = protcoords(m,1)%species
        protcoords(m,l)%linker = specieslinker(protcoords(m,1)%type+2)
...
totpoints = totpoints*2
 ...
 do m = 1,topchains,1 !nprotein-nclients
     ...
     read(21,*)BIN,BIN,BIN,BIN,BIN,BIN,BIN,BIN
        protcoords(m,1)%species =protcoords(m,1)%type/2
protcoords(m,1)%type = speciestype(protcoords(m,1)%species,1)
     ...
     do l = 2,maxl
        ...
        protcoords(m,l)%linker = specieslinker(protcoords(m,1)%type)
...
if(protcoords(m,l)%type==3) goto 878    
...
if(m<=nscaffold) then
 dx = min(abs(protcoords(m,l)%x - protcoords(m,l-1)%x),gridsize-abs(protcoords(m,l)%x &
             ...
             dz = min(abs(protcoords(m,l)%z - protcoords(m,l-1)%z),gridsize-abs(protcoords(m,l)%z &
              ...
              sumdebug = sqrt(real((dx**2) + (dy**2) + (dz**2)))
             if(sumdebug > protcoords(m,1)%linker) then
                write(6,*) 'SPLIT Scaff',m,l,l-1,sumdebug
                finalfail = .true.
             end if
          end if
...
if(m>nscaffold) then
 dx = min(abs(protcoords(m,l)%x - protcoords(m,l-1)%x),gridsizex-abs(protcoords(m,l)%x &
             ...
             dz = min(abs(protcoords(m,l)%z - protcoords(m,l-1)%z),gridsize-abs(protcoords(m,l)%z &
              ...
              sumdebug = sqrt(real((dx**2) + (dy**2) + (dz**2)))
             if(sumdebug > protcoords(m,1)%linker) then
                write(6,*) 'SPLIT client',m,l,l-1,sumdebug
                finalfail = .true.
             end if
          end if
end if
...
nclients = 0
    ...
    DO
       ...
       CASE ('TOTTYPES')
        ...
        allocate(speciestype(nspecies,maxlengthstart))
       ...
       CASE ('NEWCHAIN')
          ...
          do f = 1,specieslen(runtype)
             call get_integer(dummytype)
               speciestype(runtype,f) = dummytype  
