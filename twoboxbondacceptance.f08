program move
  ...
  double precision :: totalenergy,kT,totrmsbrute
  double precision :: totchainlength
  double precision :: totiso,sumsep,bulksumsep
  double precision :: runningaveROG,runningaveEtE,chainlength,sumdebug,polymerrog,polymerl
  integer :: minl,outputrate,backtime
  ...
  integer :: seed,maxtime,reptbackward,reptforward,equilib,cltacc,cltatt,binsize,normaliser
  ...
  integer,dimension(:,:),allocatable ::study,sumclient,visit,unbound
  ...
  integer,dimension(:),allocatable::zloc,zlocb,bindinghist
  integer::boundcount,unboundcount
  integer,dimension(:),allocatable::unboundarray,boundarray
  integer::bondformacccount,bonddeleteacccount,bondformrejcount,bonddeleterejcount,avesepcount
  double precision::avesep,aveete
  character(len=50)::pathtofile


!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!DEFINITION OF!PARAMERS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! totalenergy           = running total of system energy
! kT                    = value of 1/kT for the system        
! totrmsbrute           = sum of x,y,and z displacements for calculating root mean squared
! totchainlength        = running total of all the end to end chain lengths 
! totiso                = sum of all of the isotropic interactions in the system 
! sumsep                = running total of the displacements between beads (for the cleint chain)
! bulksumsep            = running total of the displacement between beads for (client) chain in the cluster
! runningaveROG         = running total of the radius of gyration of the client chain
! runningaveETE         = runningaverage of the end-to-end distance of the (client) chain
! chainlength           = the bead separation of each when calculated
! sumdebug              = variable used in debug subroutine for the bead separations
! polymerrog            = the average radius of gyration of the (client) chains - averaged by number of chains
! polymerl              = the average end-to-end length of the (client) chains - averaged by number of chains
! minl                  = minimum length of chain in the system
! outputrate            = the number of loops of the main routine carried out between data output 
! backtime              = the number of steps carried out in the previous simulation (if restart has happened)
! lastruntime           = the number of steps in the previous run of the simulation (if the simulation is restarted)
! timebase              = integer counter of the number of loops of the main program carried out
! steplength            = in systems with cluster moves this is the size of
!                       displacement attempted for cluster translations
! macc                  = counter of the number of accepted single bead moves 
! mrej                  = counter of the number of rejected single bead moves
! roatt                 = the number of attempted roation moves 
! roacc                 = the number of rejected rotation moves
! clientlength          = the number of beads on the client
! telacc                = the number of accepted teleport moves
! telatt                = the number of attempted teleport moves
! gridsize              = the dimensions of the cube used (where two adjacent boxes are simulated this is half the
!                        x-dimension of the cuboid
! maxlength             = maximum length of the chain in the system
! reptbackward          = number of accepted reptations in the direction of the first bead in the chain
! reptforward           = number of accepted reptations in the direction of the last bead on the chain
! t                     = a counter for time
! nprotein              = number of chains in the system
! time                  = counter for the number of loops within the main program that have been completed (
!                       including the loops completed prior to the restart
! count                 = depreciated
! avecluspop            = running total of the number of chains found within a cluster
! aveclusnumber         = running total of the number of clusters in teh system
! mtype                 = the number of different types of bead in the system
! gridsizex             = the x dimensions of the simulation
! seed                  = the number to seed the random number generation 
! maxtime               = total length of simulation
! equilib               = the number of equilibration loops of the main program
! cltacc                = the number of cluster translations accepted
! cltatt                = the number of cluster translations attempted
! binsize               = the number of bins in any histograms produced
! normaliser            = the count of the number of available binding sites -
!                       used to normalise the RDF
! successful            = count of the successful reptation moves
! reject                = count of the rejected reptation moves
! maxlength1            = maxlength of species 1
! maxlength 2           = maxlength of species 2
! q,d,u,sm,qt,ft,gt     = all integers for use within loops
! scans                 = was used to count the number of loops of move attempts
!                       but is deprecated and time carries out the same job
! nclients              = the number of client species in the system
! phasecount            = the number of times the phase subroutine is called
! rerej                 = the number of rejected reptation moves
! reacc                 = the number of accepted reptation moves
! nprotein1             = the number of type 1 chains in the system
! nprotein2             = the number of type 2 chains in the system
! nspecies              = the number of different types of chains in the system 
! vtype                 = the number of different types of chain in the system
! freezetime            = the number of snapshots to 'skip' before reading in
!                       the configuration from an external vtf file     
! nclientspecies        = the number of different types of clients 
! study                 = array of cient beads with value 1 if bound and 0 if unbound
! sumclient             = not used
! visit                 = array counting the number of individual times each bead is bound
! unbound               = array counting the number of times a client bead is unbound 
! chlen                 = array of all of the chain lengths
! speciespop            = array of the number of each species of chain
! specieslen            = array of the length of each speceis of chain
! clientlist            = is a list of the chain number given to the clients in the system
! specieslinker         = an array of the linker lengths of each species of chain
! clnos                 = array used to assign the chains to their relevant clusters and generate the
! sizes of clusters in the system
! totalbindevents       = count of the number of times that the client is bound
! (on the lefthand side, the right hand side and unbound)
! count_rate            = the precision of the timer
! count_max             = gives the maximum count for the time calculator
! start                 = the start point
! finish                = the end time
! midpoint              = a time at the midpoint of the sim 
! trackx,tracky and trackz = total x,y and z displacement of the chain centres of mass
! totdire               = the sum of directional interactions
! r                     = the radius
! chen                  = the energy of each of the chains (as calculated in the energy subroutine) 
! interenergy[a,b]      = the directional interaction energy between beads of types a and b
! interen[a,b]          = the isotropic interaction energy between beads of types a and b        
! intraenergy[a,b]      = the directional interaction energy between beads of type and b on the same chain
! locb                  = count of the number of binding events on the RHS and LHS of the box
! loc                   = count of the number of times the COM of the client is in the RHS and LHS of the box
! summing               = not used
! vdist                 = x coordinate distribution of the client beads durign the simulation
! freesites             = array of the x coordinate of the unbound beadss at the start of the simulation
! clusdist              = distribution of the different beads types from the centre of the largest cluster
! fail                  = logical which if true indicates that there has been a fundamental fail in the code 
! finalfail             = if true code has failed - this kills the simulation
! debugyes              = logical on whether to run the debug subroutine
! film                  = if true the configuration of the chains are outputted to a trajectory file
! isbond                = if true there are no isotropic interactions in the system
! clt                   = if true there are cluster translation moves
! clr                   = if true there are cluster rotation moves
! noinfo                = if true then only the scaling info is recorded - a limited simulation is run 
! scalinginfo           = if true the end-to-end and ROG properties of the chains are measured and output
! nobonds               = if true there are no bonds in the system
! restart               = if true the simulation is restarting from previous configuration
! choose1               = random number generated
! suffclust             = if true the largest cluster is coloured differently in the trjectory output 
! settingup             = this is true until the setup is complete
! twosys                = if true code reads in configurations for the left hand
!                       cube and the right hand box, if false only left hand box configuration is read in
! pointless1            = count of the number of teleport attempts to the Left hand box
! pointless2            = count of the number of teleport attempts to the right hand box
! dright                = the number of scaffold beads in the right hand box
! dleft                 = the number of scaffold beads in the left hand box
! noROGS                = the number of times the ROG is calculated ot get an average
! averageROG            = running total of the radius of gyration of the client chain(s)
! a111                  = 0 if unbound, 1-6 if bound where number indicates direction of bond
! a11n                  = chain number that the bead is bound to
! zloc                  = histogram of the x coordinate of the client beads
! zlocb                 = histogram of the x coordinate of the bound client beads
! bindinghist           = histogram of the number of client beads bound at any one time
! boundcount            = number of client receptors pair bound
! unboundcount          = number of client receptor pairs unbound
! unboundarray          = array of the unbound client receptor pairs
! boundarray            = array of the bound client receptor pairs
! bondformacccount      = count of the bond formation moves
! bonddeleteacccount    = count of the bond deletion moves
! bondformrejcount      = count of the rejected bond formation moves
! bonddeleterejcount    = count of the rejected bond deletion moves
! avesepcount           = number of times the bead separation is calculated
! avesep                = running total of the average separation between the client beads
! aveete                = running total of the average end-to-end separation of the client
! pathtofile            = the path to the input confuration of scaffold beads

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!


!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! protien (x coordinate, y coordinate, z coordinate, type of bead,
!         neighbouring bead chains[m] and beads[l] in the 6 directions
!         direction of bond, chain bead is bound to)       
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! protcoords            = all of the information for each bead int eh simulation
! client                = all of the information for the client beads
! com                   = array of all of the chain centres of mass
! dpcomclusterinit      = initial com of the system
! cell                  = array of the whole lattice giving the chain number of
!                       every bead on each lattice point
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
allocate(loc(2))
allocate(cell(gridsizex,gridsize,gridsize))
allocate(bindinghist(clientlength+1))
allocate(zloc(gridsizex))
allocate(zlocb(gridsizex))
...
!!!!!!!!!!!!!!!!!!!!!!!!SETTING UP THE SYSTEM FROM THE INPUT TRAJECTORY!!!!!!!!!
...
dpcomclusterinit%x = real(gridsizex)/2
...
write(6,*) 'LOC 1',loc(:),gridsize,gridsizex
...
!!!!!!!!!!!!!!Subroutine to find all of the client-receptor pairs and determine
!if they are bound or not!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!CHECKS IF THERE IS A NEIGHBOUR OF THE CLIENT BEAD THEN DETERMINES
!IF ITS BOUND OR UNBOUND!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!!!!!!!!Subroutine to attempt to form a bond!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!CHOOSE BOND TO ATTEMPT TO FORM!!!!!!!!!!!!!!!!!!!!!
formlogic=.false.
...
if(protcoords(m,l)%a111>0) then
!!!!!!!!!!!!!!bead is already bound so reject!!!!!!!!!!!!!!!
...
SELECT CASE (dir)
!!!!!!!!finds the neighbouring bead the client is bound to!!!!!!!!!!!!!!!!
CASE (1)
g=protcoords(m,l)%am
f=protcoords(m,l)%al
dir2=2
...
if(g==m) then
!!!!!!!!!!!!!!!!!!!!!reject if the bead is on the same chain!!!!!!!!!!!!!!
bondformrejcount= bondformrejcount+1
return
end if
...
!!!!!!!!!!!!!!reject if the recptor bead is already bound!!!!!!!!!!!!!!!!!
bondformrejcount= bondformrejcount+1
return
end if
...
!!!!!!!!!!!!!!!!reject if the neighbouring beads are the same type
!as the interaction strength will be zero!!!!!!!!!!!!!!!!!!!!!!1
bondformrejcount= bondformrejcount+1
return
end if
...
!!!!!!!!!!!!!!!!!!!!!!!1FORM BOND SO UPDATE EVERYTHING!!!!!!!!!!!!!!!!!!!
    boundcount=boundcount+1
    unboundcount=unboundcount-1
...
totalenergy=totalenergy+deltaenergy
  ...
  subroutine cleanup
!!!!!!!!!!!!!!SUBROUTINE CALLED AT THE END OF THE SIM TO OUTPUT ALL OF THE DATA!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!!SUBROUTINE TO SET UP THE CELL ARRAY WITH ALL OF THE BEAD NUMBERS
!ON THE LATTICE SITES, CELL(X,Y,Z) IS 0 IF THE LATTICE SITE IS EMPTY!!!!!!
...
!!!!!!!!!!!READS IN SOME PARAMETERS AFTER THE RESTART!!!!!!!!!!!!!!!!!!!!!!!
    ...
    subroutine sepcalc
!!!!!!!!!!!!!!!!!!!CALCULATES THE SEPARATION BETWEEN BEADS ON THE !CLIENT!!!!!!!!!!!!
    integer::m,l
...
!!!!!!!!!!!CALCULATES X DISTANCE BETWEEN BEADS AND ACCOUNTS FOR PBCS!!!!!!!!!
...
!!!!!!!!!!!!!!SUBROUTINE TO DO A SINGLE BEAD MOVE - FOR A MONOVALENT CLIENT!!!!!!!!!!!!!!!!!!!!!!!!!!
    integer,intent(in)::m
...
if(tele> 0.0) then
!!!!!!!!!!!!!!!!!!!!!METHOD FOR CHOOSING BEAD DISPLACEMENT WHICH INVOLSES A LOT
!OF REJECTED MOVES SO NOT CALLED - HENCE TELE SET TO 0.5!!!!!!!!!!!!!!!!
...
15  continue
    ...
    tempcoord(l)%x = modulo(protcoords(m,l)%x+dx-1,gridsizex)+1
...
!!!!!!!!!!!!PICK A RANDOM LOCATION TO MOVE THE BEAD TO!!!!!!!!!!!!!!!!!!!!
         tempcoord(l)%x = int(ran2(seed)*gridsizex)+1
...
!!!!!!!!!!!!!!!!!IF THE TRIAL POSITION IS ALREADY OCCUPIED, REJECT!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!!!CHECK FOR NEW NEIGHBOURS!!!!!!!!!!!!!!!!!!!!!!!!!
                if((tempcoord(l)%x < gridsize).or.(tempcoord(l)%x>(gridsizex-1))) then
if(cell(modulo(tempcoord(l)%x,gridsizex)+1,tempcoord(l)%y,tempcoord(l)%z)%bm/=0)then
                pr=cell(modulo(tempcoord(l)%x,gridsizex)+1,tempcoord(l)%y,tempcoord(l)%z)%bm
                str=cell(modulo(tempcoord(l)%x,gridsizex)+1,tempcoord(l)%y,tempcoord(l)%z)%bl
        bdir = -1
...
if(cell(modulo(tempcoord(l)%x-2,gridsizex)+1,tempcoord(l)%y,tempcoord(l)%z)%bm/=0)then
                pr=cell(modulo(tempcoord(l)%x-2,gridsizex)+1,tempcoord(l)%y,tempcoord(l)%z)%bm
                str=cell(modulo(tempcoord(l)%x-2,gridsizex)+1,tempcoord(l)%y,tempcoord(l)%z)%bl
        bdir = 1
                      call bondforminter(m,pr,l,str,tempcoord,bdir,deltaenergy)
end if
end if

if(tempcoord(l)%x<gridsize+1) then
...
!!!!!!!!!!!!!!!!!!!!!CALL METROPOLIS ALGORITHM FOR REJECT/ACCPETANCE!!!!
...
call comfind(m,.false.)
  if(com(1,m)%x >=gridsize+1.0) then
     loc(2) = loc(2)+1
	lc = .false.
  else if(com(1,m)%x <gridsize+1.0) then
     loc(1) = loc(1)+1
	lc = .true.
     end if
...
zloc(protcoords(m,l)%x) = zloc(protcoords(m,l)%x) + 1
                      if(protcoords(m,l)%a111>0) then
zlocb(protcoords(m,l)%x) = zlocb(protcoords(m,l)%x) + 1
...
if(control >0) then
!left = .true.
	if(lc .eqv. .true.) left = .true.
	if(lc .eqv. .false.) left = .false.
...
if(study(a,g) == 1) call rdf(m,g,left)
end do
...
if(study(a,g) ==0) unbound(a,g) =unbound(a,g)+1
 ...
 !call comlargecomp(clsize,dummycom,checklist,route,rcounter,comx,comy,comz,toolarge)
    ...
    do m = 1,nprotein
       maxl = chlen(m)
       do l = 1,maxl
          sumsq%x = min(abs(protcoords(m,l)%x-comcluster%x),gridsizex-abs(protcoords(m,l)%x-comcluster%x))
    ...
    do g = 1,clsize
       ...
       do l = 1,maxl
          ...
          if(((protcoords(m,l)%x-comcluster%x)>maxx) .and. &
               ((protcoords(m,l)%x-comcluster%x)<(gridsizex/2))) maxx =(protcoords(m,l)%x-comcluster%x)
          if((((gridsizex+protcoords(m,l)%x)-comcluster%x)>maxx).and. &
               (((gridsizex+protcoords(m,l)%x)-comcluster%x)<(gridsizex/2))) then
          maxx =((gridsizex+protcoords(m,l)%x)-comcluster%x)
       end if
       if(((comcluster%x-protcoords(m,l)%x)>minx) .and. &
           ((comcluster%x-protcoords(m,l)%x)<(gridsizex/2))) minx =(comcluster%x-protcoords(m,l)%x)
          if((((gridsizex+comcluster%x)-protcoords(m,l)%x)>minx).and. &
               (((gridsizex+comcluster%x)-protcoords(m,l)%x)<(gridsizex/2))) then
          minx =((gridsizex+comcluster%x)-protcoords(m,l)%x)
       end if       
...
doubleprecision :: rx,ry,rz,bondx,bondy,bondz
  ...
  open(121, file = '../movetaggedtwo.vtf', action = 'read')  
...
subroutine foundation
    ...
    integer::m,l,no1,no2,maxl,z,topchains
    character(len = 10) :: BIN,nonsensea,nonsenseb,nonsensec
integer::totpoints,dummymax,g,sc
doubleprecision :: rx,ry,rz,bondx,bondy,bondz
...
topchains = 0
  ...
  write(6,*) 'nspecies',nspecies,speciespop(1),specieslen(1)
do sc=1,2,1
	topchains = topchains + speciespop(sc)
     totpoints = totpoints + (speciespop(sc)*specieslen(sc))
end do
...
open(108,file= 'bonddate.dat',action= 'read')
    do a = 1,nclients
       read(108,*)sumclient(a,:),vc,vb,unbound(a,:),totalbindevents(1),totalbindevents(2),totalbindevents(3),locb(1),locb(2),&
bindinghist(:)
    end do
close(108)
...
close(108)
  ...
  subroutine rotate(m,l,b,tempcoords,xx,xy,xz,yx,yy,yz,zx,zy,zz,dx,dy,dz)
    ...
    !write(6,*) dx,dy,dz
    tempcoords(b)%x = modulo(protcoords(m,l)%x + (xx*dx) + (xy*dy) + (xz*dz) -1,gridsizex)+1
...
if(l>1) then
    dx = min(abs(tempcoord(l)%x - protcoords(m,l-1)%x),gridsizex-abs(tempcoord(l)%x &
         - protcoords(m,l-1)%x)) 
...
19     continue
       ...
       tempcoord(1)%x = modulo(protcoords(m,1)%x + dx-1,gridsizex)+1
       tempcoord(1)%y = modulo(protcoords(m,1)%y + dy-1,gridsize)+1
       tempcoord(1)%z = modulo(protcoords(m,1)%z + dz-1,gridsize)+1

     dx = min(abs(tempcoord(1)%x - protcoords(m,2)%x),gridsizex-abs(tempcoord(1)%x &
         - protcoords(m,2)%x)) 
...
if(tempcoord(1)%x<gridsize+1) then
...
goto 83
       ...
       !if(deltaenergy<0.0)write(6,*) 'energy post intra=',deltaenergy
        ...
        if((tempcoord(1)%x < gridsize) .or. (tempcoord(1)%x>(gridsizex-1))) then

if(cell(modulo(tempcoord(1)%x,gridsizex)+1,tempcoord(1)%y,tempcoord(1)%z)%bm/=0)then
                pr=cell(modulo(tempcoord(1)%x,gridsizex)+1,tempcoord(1)%y,tempcoord(1)%z)%bm
                str=cell(modulo(tempcoord(1)%x,gridsizex)+1,tempcoord(1)%y,tempcoord(1)%z)%bl
        bdir = -1
...
if(cell(modulo(tempcoord(1)%x-2,gridsizex)+1,tempcoord(1)%y,tempcoord(1)%z)%bm/=0) then
                pr=cell(modulo(tempcoord(1)%x-2,gridsizex)+1,tempcoord(1)%y,tempcoord(1)%z)%bm
                str=cell(modulo(tempcoord(1)%x-2,gridsizex)+1,tempcoord(1)%y,tempcoord(1)%z)%bl
        bdir = 1
                      call bondforminter(m,pr,1,str,tempcoord,bdir,deltaenergy)
...
15     continue
       ...
       tempcoord(maxl)%x = modulo(protcoords(m,maxl)%x + dx-1,gridsizex)+1
       tempcoord(maxl)%y = modulo(protcoords(m,maxl)%y + dy-1,gridsize)+1
       tempcoord(maxl)%z = modulo(protcoords(m,maxl)%z + dz-1,gridsize)+1

     dx = min(abs(tempcoord(maxl)%x - protcoords(m,maxl-1)%x),gridsizex-abs(tempcoord(maxl)%x &
...
if(tempcoord(maxl)%x<(gridsize+1)) then
...
goto 83
     ...
     if((tempcoord(maxl)%x < gridsize) .or. (tempcoord(maxl)%x>(gridsizex-1)))then
        if(cell(modulo(tempcoord(maxl)%x,gridsizex)+1,tempcoord(maxl)%y,tempcoord(maxl)%z)%bm/=0)then
                pr=cell(modulo(tempcoord(maxl)%x,gridsizex)+1,tempcoord(maxl)%y,tempcoord(maxl)%z)%bm
                str=cell(modulo(tempcoord(maxl)%x,gridsizex)+1,tempcoord(maxl)%y,tempcoord(maxl)%z)%bl
        bdir = -1
...
if(cell(modulo(tempcoord(maxl)%x-2,gridsizex)+1,tempcoord(maxl)%y,tempcoord(maxl)%z)%bm/=0)then
                pr=cell(modulo(tempcoord(maxl)%x-2,gridsizex)+1,tempcoord(maxl)%y,tempcoord(maxl)%z)%bm
                str=cell(modulo(tempcoord(maxl)%x-2,gridsizex)+1,tempcoord(maxl)%y,tempcoord(maxl)%z)%bl
        bdir = 1
                call bondforminter(m,pr,maxl,str,tempcoord,bdir,deltaenergy)
end if
end if
...
if(abs(delx) == (gridsizex - 1)) bdir = (delx/(gridsizex-1))*(gridsizex-1)
    else if(delx == 0) then
       dx = 0
    end if
    if(dx == 4) then
 ...
 noROGS = noROGS+1
  ...
  subroutine length
    ...
    do m = 1,nprotein,1
        ...
        maxl = chlen(m)
        dxsum = min(abs(protcoords(m,maxl)%x - protcoords(m,1)%x),gridsizex-abs(protcoords(m,maxl)%x - protcoords(m,1)%x))
...
polymerl = polymerl + normchainl
  ...
  subroutine dataout
    ...
    write(choice,*) 'pbc', real(4*gridsizex),real(4*gridsize),real(4*gridsize)
          ...
          do a = 1,clsize-1,1
             ...
             do g = a+1,clsize
                ...
                if(cb(b,f)==1) then
                   ...
                   if (delx > (gridsizex)/2) delx = delx-gridsizex
                   if (delx < -(gridsizex)/2) delx = delx + gridsizex
                   comx(b,f) =  delx
                   comx(f,b) = -delx
...
76        if(clsize == 1) then
          ...
          !write(6,*) dummycom%x
          comcluster%x = INT(modulo((com(1,centralchain)%x) + (dummycom%x/totcluspop)-1,real(gridsizex))+1)
          ...
          dpcomcluster%x = modulo((com(1,centralchain)%x) + (dummycom%x/totcluspop)-1,real(gridsizex))+1
...
comcluster%x = int(real(gridsizex)/2)
