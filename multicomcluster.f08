...
if(track .eqv. .true.) call rms(m)
  ...
  subroutine clustercom(m,clsize,comcluster,cb,cllist)
...
!write(6,*) tcom(vv)%z
 ...
 !write(6,*) 'pass'
    ...
    if(clusen == 0.0) call updatecluspos(tempcluscoord,ctbo,ctbdi,tcom,cllist,clsize)
    !write(6,*) 'b'
  ...
  subroutine clusterrotation(m,clno,clsize,cb,cllist)
    ...
    integer,dimension(:),intent(inout) ::clno,cllist
        integer,dimension(:,:),intent(inout) ::cb
    integer :: a,b,l,pr,st,maxl,maxback,dummy,rotatechoose,xr,yr,zr,rb,chnum
    double precision :: clusen
    integer,dimension(:,:),allocatable :: deltax,deltay,deltaz
    Type(basicp),dimension(:,:),allocatable :: tempcluscoord
    type(centremass),dimension(:),allocatable:: tcom
    integer,dimension(:,:),allocatable::ctbo,ctbdi
    logical :: clusmove,clusrotpass
...
rb = cllist(b)
     ...
     call clustercom(m,clsize,comcluster,cb,cllist)
...
!write(6,*) 'cluslist',b,rb           
    ...
    do rb = 1,clsize
       ...
       call tempclustercom(m,clsize,tcom,comclustemp,cb,cllist)
...
!
  ...
  subroutine tempclustercom(m,clsize,tcom,comclustemp,cb,cllist)
    integer,intent(in)::m,clsize
    type(centremass),dimension(:),intent(inout) :: tcom
    integer,dimension(:),intent(in) :: cllist
        integer,dimension(:,:),intent(inout) ::cb
