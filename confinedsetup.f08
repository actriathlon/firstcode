program setup
  ...
  integer,dimension(:),allocatable :: specieslen,speciespop,link
  logical :: scalinginfo
   type protein
...
15          if (control ==1) then
               ...
               protcoords(m,l)%type = master(speciesselect,l)
                ...
                write(19,*) protcoords(m,1)%species ,    protcoords(m,1)%x,    protcoords(m,1)%y, &
                     protcoords(m,1)%z,protcoords(m,1)%type,maxl,protcoords(m,1)%linker
