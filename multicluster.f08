program move
  ...
  do time = 1,maxtime,1
     ...
     if(modulo(time,100) == 0) write(93,*) time,totalenergy
...
contains
  ...
  subroutine vectorspin(m,maxl)
    ...
    call intervector(m,1,maxl,tempcoord,tempintbond,deltaenergy,tbo) !error here


    if(Energydecision(deltaenergy) .eqv. .True.) then
       totalenergy = totalenergy + deltaenergy

       call updatepos(m,1,maxl,tempcoord,tbo)
       call updateintrabondvector(m,tempisobond,maxl)
       call updateinterbond(m,1,maxl,tempintbond)
    else 
       continue
    end if
  ...
  subroutine intervector(chain1,beadmin,beadmax,tempc,tempintbond,deltaenergy,tbo)
    ...
    !write(6,*) 'interallocate'
    do chain2 = 1,chain1-1,1
       do bead1 = beadmin,beadmax,1
          !write(6,*) 'species',protcoords(chain2,1)%species,chain1,chain2,bead1,tempc(bead1)%x
  ...
  subroutine foundation
    ...
    do m = 1,nprotein,1
       ...
       !if(maxl<maxlength) then
       ! do z =maxl+1,maxlength
       !protcoords(m,z)%species = protcoords(m,1)%species
       !protcoords(m,l)%x =0
       !protcoords(m,l)%y=0
       !protcoords(m,l)%z =0
       !end do
  ...
  subroutine pickmoves
    ...
    do scans = 1,(nprotein1*maxlength1 + nprotein2*maxlength2)
       ...
       if(protcoords(m,1)%species == 1) maxl = maxlength1
       if(protcoords(m,1)%species == 2) maxl = maxlength2
       if(decide <= 8.0/11) call vectorspin(m,maxl)
...
71  if(cranksep .eqv. .true.) then
    ...
    call intrabondallocatelower(m,l+1,p-1,p,maxl,tempcoord,tempisobond,deltaenergy,tbo)
    ...
    if(Energydecision(deltaenergy) .eqv. .True.) then
       ...
       call updateintrabond(m,l+1,p-1,p,maxl,tempisobond)
...
43  if (crankcont .eqv. .false.) then
  ...
  subroutine rightanglemove
    ...
    if (rac .eqv. .true.) then    
       ...
       call intrabondallocatelower(m,l,l,l+3,maxl,tempcoord,tempisobond,deltaenergy,tbo)
       ...
       if(Energydecision(deltaenergy) .eqv. .true.) then
          ...
          call updateintrabond(m,l,l,l+3,maxl,tempisobond)
...
37  if (rac .eqv. .false.) then
  ...
  subroutine pivot(m,l,maxl)
    ...
    if(l > maxl/2) then
       ...
       !write(6,*) 'pree intra'
       call intrabondallocateupper(m,l+1,maxl,1,l,tempcoord,tempisobond,deltaenergy,tbo)

       !write(6,*) 'pass'
       call interbondallocate(m,l+1,maxl,tempcoord,tempintbond,deltaenergy,tbo)

       !write(6,*) 'pass2'
       if(Energydecision(deltaenergy) .eqv. .true.) then
          totalenergy = totalenergy + deltaenergy
          call updatepos(m,l+1,maxl,tempcoord,tbo)
          call updateintrabond(m,l+1,maxl,1,l,tempisobond)
          call updateinterbond(m,l+1,maxl,tempintbond)
          successful = successful + 1
          call counts(0,0,1,0)
       ...
       call intrabondallocatelower(m,1,l-1,l,maxl,tempcoord,tempisobond,deltaenergy,tbo)
       call interbondallocate(m,1,l-1,tempcoord,tempintbond,deltaenergy,tbo)

       if(Energydecision(deltaenergy) .eqv. .true.) then
          totalenergy = totalenergy + deltaenergy
          call updatepos(m,1,l-1,tempcoord,tbo)
          call updateintrabond(m,1,l-1,l,maxl,tempisobond)
...
75  if (pivcont .eqv. .false.) then
  ...
  subroutine interbondallocate(chain1,beadmin,beadmax,tempc,tempintbond,deltaenergy,tbo)
    ...
    do chain2 = 1,chain1-1,1
       do bead1 = beadmin,beadmax,1
          ...
          if(protcoords(chain2,1)%species == 2) maxback = maxlength2
          do bead2 = 1,maxback,1
             if(intbond(chain2,chain1,bead2,bead1) == 1) deltaenergy = deltaenergy - &
                  interenergy(protcoords(chain1,bead1)%species,protcoords(chain2,bead2)%species)
             call adjacent(chain2,bead1,bead2,tempc,adjver,bdir)
             if((adjver .eqv. .true.) .and. (bdir == -1*tbo(bead1)) &
                  .and. (bdir ==bonddd(chain2,bead2))) then
  ...
  subroutine clusterassign(chainnum,clno,clsize)
    ...
    integer:: m,l,g,f,clcount,bdir,maxlengthss,maxback
  ...
  subroutine reptation
    ...
    if (choose > 0.0) then
       ...
       if(Energydecision(deltaenergy) .eqv. .true.) then
          ...
          call updateintrabond(m,1,1,4,maxl,tempisobond)
       ...
       if((direc == g1) .and. (g1 == 1)) then
          dx = 1
          call bonddirection(5,1,8,tbo(maxl))
       else if((direc ==(g1 +g2)) .and.  (g2 == 1)) then
          dx = -1
          call bonddirection(5,-1,8,tbo(maxl))
       else if((direc == (g1+g2+g3)).and.  (g3 == 1)) then
          dy = 1
          call bonddirection(5,2,8,tbo(maxl))
       else if((direc == (g1+g2+g3+g4)) .and.  (g4 == 1)) then
          dy = -1
          call bonddirection(5,2,8,tbo(maxl))
       else if((direc == (g1+g2+g3+g4+g5)) .and.( g5 == 1)) then
          dz = 1
          call bonddirection(5,3,8,tbo(maxl))
       else if((direc == (g1+g2+g3+g4+g5+g6)).and.  (g6 == 1)) then
          dz = -1
          call bonddirection(5,-3,8,tbo(maxl))
       end if
       ...
       do st = 2,maxl-2
          ...
          if((adjver.eqv. .true.) .and. (bdir == -1*tbo(maxl)) &
       ...
       if(Energydecision(deltaenergy) .eqv. .true.) then
          ...
          call updatepos(m,maxl,maxl,tempcoord,tbo)
          call updateintrabond(m,maxl,maxl,1,maxl-3,tempisobond)
          call updateinterbond(m,maxl,maxl,tempintbond)
          successful = successful + 1
...
83  if (reptcont .eqv. .false.) then
    ...
    !if(time == 700) write(67,*) 'time',time
    !if(time == 700) call dataout
  ...
  subroutine comfind
    integer :: dcomx,dcomy,dcomz,maxl
             ...
             do l = 1,maxl,1
                ...
                do f = 1,maxlengthss,1
                   ...
                   if(adjver.eqv. .true. .and. bdir == -1*ctbo(a,l) .and. &
                        bdir == ctbo(g,f)) energyofcluster = energyofcluster + &
                        interenergy(protcoords(a,l)%species,protcoords(g,f)%species)
                end do
             end do
...
87     if(bm .eqv. .true.) continue
  ...
  subroutine clusterrotation(m,clno,clsize)
    ...
    integer,intent(inout)::clsize
    integer,dimension(:),intent(inout) ::clno
    integer :: a,b,l,pr,st,maxl,maxback
    ...
    do a = 1,nprotein
       if(clno(a) == clno(m)) then
          ...
          if(rotatechoose <= 1.0/9) then
             call rotatex(b,1,-1,deltay,deltaz,tempcluscoord,ctbo,maxl)
          else if(rotatechoose > 1.0/9 .and. rotatechoose <= 2.0/9) then
             call rotatex(b,-1,1,deltay,deltaz,tempcluscoord,ctbo,maxl)
          else if(rotatechoose > 2.0/9 .and. rotatechoose <= 1.0/3) then
             call clusterflip(b,1,-1,1,deltax,deltay,deltaz,tempcluscoord,ctbo,maxl)    
          else if(rotatechoose > 1.0/3 .and. rotatechoose <= 4.0/9) then
             call rotatey(b,1,-1,deltax,deltaz,tempcluscoord,ctbo,maxl)
             ...
             call rotatez(b,1,-1,deltax,deltay,tempcluscoord,ctbo,maxl)
...
31  if(clusmove .eqv. .false.) continue
  ...
  subroutine clusterflip(protno,cx,cy,cz,deltax,deltay,deltaz,tempcluscoord,ctbo,maxl)
    ...
    if(cx == -1) call clusrbond(protno,ctbo,1,0,maxl)    
    if(cy == -1) call clusrbond(protno,ctbo,2,0,maxl)
    if(cz == -1) call clusrbond(protno,ctbo,3,0,maxl)   


  end subroutine clusterflip

  subroutine rotatex(protno,cy,cz,deltay,deltaz,tempcluscoord,ctbo,maxl)
    ...
    if(cy ==-1) call clusrbond(protno,ctbo,1,1,maxl)
    if(cz == -1) call clusrbond(protno,ctbo,1,-1,maxl)

  end subroutine rotatex

  subroutine rotatey(protno,cx,cz,deltax,deltaz,tempcluscoord,ctbo,maxl)
    ...
    if(cx ==-1) call clusrbond(protno,ctbo,2,1,maxl)
    if(cz == -1) call clusrbond(protno,ctbo,2,-1,maxl)


  end subroutine rotatey

  subroutine rotatez(protno,cx,cy,deltax,deltay,tempcluscoord,ctbo,maxl)
    ...
    if(cx ==-1) call clusrbond(protno,ctbo,3,-1,maxl)
    if(cy == -1) call clusrbond(protno,ctbo,3,1,maxl)
...
31  if(clusmove .eqv. .false.) moveallow = .false.
  ...
  subroutine debug
    integer::m,l,f,g,dx,dy,dz,maxlengthss,maxl
  ...
  subroutine clustercount
    integer :: m,l,g,f,clcount,clusterpop,bdir,maxlengthss,maxl
