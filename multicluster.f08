program move
  ...
  integer :: successful,reject,pivotlimit,maxpiv,maxlength1,maxlength2,q,d,u,sm
  integer:: cracc,crrej,raacc,rarej,rerej,reacc,pvacc,pvrej,nprotein1,nprotein2
  integer,dimension(:,:),allocatable :: bonddd,bdi
  double precision :: crrat,emrat,rarat,pvrat,rerat,intraen,interen
...
!sets the limit of the pivot move
  ...
  do time = 1,maxtime,1
     call CPU_TIME(start)
     fail = .false.
     if ((mod(time,10000) == 0) .and. (film .eqv. .true.) ) then
         call dataout
        !call energy
        !call bondcheck
     end if
...
!write(6,*) 'clustercount'
     ...
     !write(6,*) 'gfd'
     if (modulo(time,1) == 0 .and. debugyes .eqv. .true.) then
        !write(6,*) 'debug start'
        call debug
        !write(6,*) 'debug fin'
     else
     ...
     !if(modulo(time,100000) == 0) write(13,*) (finish-start)*100
...
if((bdi(m,l) == 0) .or. (abs(bdi(m,l)) >3)) write(6,*) 'fail',m,l
  ...
  subroutine clusterposition
    ...
    if((1.0/clsize) > (0.1*ran2(seed))) then
       ...
       if(decider > 0.0) then
          ...
          !if(time> 10 .and. (time < 2000)) write(6,*) 'translate',time
       else if(decider <= 0.0) then
          call clusterrotation(m,clno,clsize)
           !if(time> 10 .and. (time < 2000)) write(6,*) 'rotate',time
       end if
       !end do
       !write(6,*) 'clustermove'
       !call clusterassign(m,clno,clsize)
  ...
  subroutine pickmoves
    ...
    !do scans = 1,(nprotein1*maxlength1 + nprotein2*maxlength2)
       ...
       if(protcoords(m,1)%species == 1) maxl = maxlength1
       if(protcoords(m,1)%species == 2) maxl = maxlength2
       if(decide <= 6.0/10) call vectorspin(m,maxl)
       if(decide > 6.0/10 .and. decide <= 8.0/10) call positioning
       if(decide > 8.0/10 .and. decide <= 1.0) call clusterposition
  ...
  subroutine positioning
    ...
    !if(time == 1000) write(6,*) 'crank'
       ...
       !if(time == 1000) write(6,*) 'pivot',m,l
       call pivot(m,l,maxl)
    end if
...
!write(6,*) 'a',beadmin,beadmax
  ...
  subroutine updatecluspos(clno,tempcluscoord,chnum,ctbo,ctbdi)
    !updates bead positions
       ...
       do pr = m+1,nprotein,1
          ...
          do st = 1,maxback,1
             ...
             if((adjver.eqv. .true.)) then
                deltaenergy = deltaenergy + interen
                tbr(pr,maxl,st) = 1
                if((bdir == -1*tbo(maxlength)) .and. (bdir ==bonddd(pr,st))) then
                deltaenergy = deltaenergy + interenergy(protcoords(m,1)%species,protcoords(pr,st)%species)
                tempintbond(pr,maxl,st) = 1
             else
                tempintbond(pr,maxl,st) = 0
...
83  if (reptcont .eqv. .false.) then
  ...
  subroutine clustercom(m,clno,clsize)
    integer,intent(in)::m
    integer,dimension(:),intent(inout) ::clno
    integer ::  a,clsize,b,base,maxl,totcluspop
    ...
    !write(6,*) 'stat'
    comcluster%x = int(((modulo(com(t,m)%x + comx/2 -1,real(gridsize)))+1)/totcluspop)
    !write(6,*) 'stat'
    comcluster%y = int(((modulo(com(t,m)%y + comy/2 -1,real(gridsize)))+1)/totcluspop)
    !write(6,*) 'stat'
    comcluster%z = int(((modulo(com(t,m)%z + comz/2 -1,real(gridsize)))+1)/totcluspop)
!write(6,*) 'fin'
    !if(clsize ==1) then
...
31  if(dx + dy + dz ==1) then
  ...
  subroutine dataout
    integer::m,l,maxl,f,g,maxback
    type(rprot),dimension(:,:),allocatable :: bondposit
 
allocate(bondposit(nprotein,maxlength))
    !write(6,*) 'data'
    
!write(67,*) 2*((nprotein1*maxlength1) + (nprotein2*maxlength2))
write(67,*) ((nprotein1*maxlength1) + (nprotein2*maxlength2))
    ...
    do m = 1,nprotein,1
       ...
       do l = 1,maxl,1
          ...
          do f= l,maxl
             if((isobond(m,l,f) ==1) .or. (ba(m,l,f) ==1)) then
                !write(67,*) 'O', 2*bondposit(m,l)%x, 2*bondposit(m,l)%y, &
               !2*bondposit(m,l)%z
                goto 71
else if((isobond(m,f,l) ==1) .or. (ba(m,f,l) ==1)) then
          !write(67,*) 'O', 2*bondposit(m,l)%x, 2*bondposit(m,l)%y, &
               !2*bondposit(m,l)%z
          goto 71           
       end if
    ...
    do g = m,nprotein
       ...
       if(protcoords(g,1)%species == 2) maxback = maxlength2 
          do f= 1,maxback
             if((intbond(m,g,l,f) ==1) .or. (br(m,g,l,f) ==1)) then
          !write(67,*) 'O', 2*bondposit(m,l)%x, 2*bondposit(m,l)%y, &
               !2*bondposit(m,l)%z
          goto 71
            else if((intbond(g,m,f,l) ==1) .or. (br(g,m,f,l) ==1)) then
          !write(67,*) 'O', 2*bondposit(m,l)%x, 2*bondposit(m,l)%y, &
               !2*bondposit(m,l)%z
          goto 71
       end if
          ...
          !write(67,*) 'H', 2*bondposit(m,l)%x, 2*bondposit(m,l)%y, &
               !2*bondposit(m,l)%z
...
87     if(bm .eqv. .true.) continue
 ...
 !if(time == 1166) write(6,*) 'hereheee'
    ...
    !if(moveallow .eqv. .true.) then
       ...
       if(clusen == 0.0)  call updatecluspos(clno,tempcluscoord,m,ctbo,ctbdi)
    !end if
  ...
  subroutine clusterrotation(m,clno,clsize)
    ...
    integer,intent(inout)::clsize
    integer,dimension(:),intent(inout) ::clno
    integer :: a,b,l,pr,st,maxl,maxback,dummy
...
!write(6,*) rotatechoose
              do b = 1,nprotein
                 if(clno(b) == clno(m)) then
                    !write(6,*) 'clnos',clno(b),clno(m)
                    !if(time> 318 .and. (time < 330)) then
                    !dummy = dummy + 1
                    !write(6,*) 'dummy',dummy
                    !end if
                    ...
                    if(rotatechoose <= 1.0/9) then
                       call rotatex(b,1,-1,deltay,deltaz,tempcluscoord,ctbo,maxl,ctbdi)
                    else if(rotatechoose > 1.0/9 .and. rotatechoose <= 2.0/9) then
                       call rotatex(b,-1,1,deltay,deltaz,tempcluscoord,ctbo,maxl,ctbdi)
                    else if(rotatechoose > 2.0/9 .and. rotatechoose <= 1.0/3) then
                       call clusterflip(b,1,-1,-1,deltax,deltay,deltaz,tempcluscoord,ctbo,maxl,ctbdi)    
                    else if(rotatechoose > 1.0/3 .and. rotatechoose <= 4.0/9) then
                       call rotatey(b,1,-1,deltax,deltaz,tempcluscoord,ctbo,maxl,ctbdi)
                       ...
                       call rotatez(b,1,-1,deltax,deltay,tempcluscoord,ctbo,maxl,ctbdi)
...
!if(ctbdi(m,s) == 0) write(6,*) 'bdi faillll'
  ...
  subroutine clusterflip(protno,cx,cy,cz,deltax,deltay,deltaz,tempcluscoord,ctbo,maxl,ctbdi)
  ...
  subroutine rotatex(protno,cy,cz,deltay,deltaz,tempcluscoord,ctbo,maxl,ctbdi)
...
!flipped rotation
  ...
  subroutine rotatey(protno,cx,cz,deltax,deltaz,tempcluscoord,ctbo,maxl,ctbdi)
...
!write(6,*) 'aaaaaa'
  ...
  subroutine rotatez(protno,cx,cy,deltax,deltay,tempcluscoord,ctbo,maxl,ctbdi)
  ...
  subroutine clustermove(clno,chainno,delx,dely,delz,tempcluscoord,moveallow,ctbo,ctbdi)
    ...
    do b =1,maxl
       ...
       do pr = 1,nprotein
          ...
          if(protcoords(pr,1)%species == 2) maxback = maxlength2 
          do st = 1,maxback,1
             !if(t > 400 .and. (pr == 14) .and. (st == 4) .and. (chainno == 12) .and. (b ==1)) then
                !write(6,*) tempcluscoord(chainno,b)%x,tempcluscoord(chainno,b)%y,tempcluscoord(chainno,b)%x &
                     !,protcoords(pr,st)%x,protcoords(pr,st)%y,protcoords(pr,st)%x
                !end if
  ...
  subroutine clustercount
    ...
    do m = 1,nprotein
       ...
       if(clnos(m) /=m) then
          ...
          if(ANY(cllist(:) == clnos(m))) then
             continue
          else
             clustcount = clustcount + 1
             cllist(a) = min(clnos(m),clnos(g))
             a = a+1
          end if
       end if
    end do
    ...
    if(clustcount /= 0) write(82,*) time,clustcount, clusterpop, (clusterpop/clustcount) !,clcount
