...
!interenergy = -1.0
  ...
  do timebase = 1,maxtime,1
     ...
     if(modulo(time,outputrate) ==0) call energy(.FALSE.)
...
17     maxl = chlen(m)
  ...
  subroutine crankshaftmove
    ...
    integer :: dummy,dummy2,p,s,m,l,str,st,pr,sign,probs,maxl,maxback,bdir
    ...
    do st = l,p,1
       ...
       call removeenergyintrapiv(m,st,deltaenergy,tempcoord,l,p)
...
51  if(racc .eqv. .true.) continue
 ...
 !      write(6,*) 'reality',protcoords(370,29)%bm,protcoords(370,29)%bl,l
       do st = l,maxl,1
          ...
          tempcoord(st)%fm = 0
  base = min(st-3,l)
          call removeenergyintrapiv(m,st,deltaenergy,tempcoord,base,maxl)
...
!return
       ...
       do st = 1,l-1,1
          ...
          tempcoord(st)%fm = 0
          base2 = max(st+3,l-1)
          call removeenergyintrapiv(m,st,deltaenergy,tempcoord,1,base2)
          call removeenergyinter(m,st,deltaenergy,tempcoord)
base = max(st+3,l)
          do str = base,maxl,1
             call adjacent(m,st,str,tempcoord,adjver,bdir)
...
75  if (pivcont .eqv. .false.) then
   ...
   subroutine removeenergyintrapiv(m,l,deltaenergy,tempcoord,exmin,exmax)
    integer,intent(in)::m,l,exmin,exmax
...
!check this and cluster bonds update
  ...
  subroutine updatebondother(m,beadmin,beadmax,tempcoord)
...
if(bg /=0) then
       ...
       if(tempcoord(l)%em == -1) then
          ...
          protcoords(m,l)%el = tempcoord(l)%el
             ...
             if(bg /= 0) then
           !     write(6,*) 'test',bg,bf,g,f
                if((bg /= g) .or. (bf /=f)) then
            !       write(6,*) 'success',bg,bf
...
!call checking(9)
 ...
 !call checking(1)
             call updatebondother(m,maxl,maxl,tempcoord)
 !call checking(2)
...
!call checking(10)
          ...
          if(Energydecision(deltaenergy) .eqv. .true.) then
             ...
             do l =1,maxl-1,1
                ...
                !call checking(1)
                call updatebondother(m,1,1,tempcoord)
                !call checking(2)
...
83        if (reptcont .eqv. .false.) then
 ...
 !         del2 = -abs(del)
          ...
          if(tempcoord(st)%am >0) then
             ...
             if(g == m) then
             f = tempcoord(st)%al
             deltaenergy = deltaenergy + interenergy(protcoords(m,st)%type,protcoords(g,f+del)%type)
             deltaenergy = deltaenergy - interenergy(protcoords(m,st)%type,protcoords(g,f)%type)
          end if
          end if
...
if(g == m) then
          ...
          if(protcoords(m,l)%am >0) then             
             ...
             if(g == m) then
                if(f /=buzz) then
             deltaenergy = deltaenergy + interenergy(protcoords(m,l+del2)%type,protcoords(g,f+del2)%type)
             deltaenergy = deltaenergy - interenergy(protcoords(m,l)%type,protcoords(g,f)%type)
             end if
          else if(g/=m) then
...
53        if(compass .eqv. .true.) continue
        ...
        subroutine energy(init)
          ...
          if(init .eqv. .true.) then
             ...
             !maxl = chlen(4)
             !do l = 1,maxl
             !write(6,*) '4 info', protcoords(4,l)
!end do
             !write(6,*) 'information', bonddd(18,5),bonddd(14,3),protcoords(18,5)%x,&
               !protcoords(18,5)%y,protcoords(18,5)%z,protcoords(14,3)%x,&
               !protcoords(14,3)%y,protcoords(14,3)%z
                    !write(6,*) 'information', bonddd(4,2),bonddd(12,4),protcoords(4,2)%x,&
               !protcoords(4,2)%y,protcoords(4,2)%z,protcoords(12,4)%x,&
               !protcoords(12,4)%y,protcoords(12,4)%z
          !write(6,*) 'basic', protcoords(5,2)%em,protcoords(5,2)%el,&
          !protcoords(10,1)%fm,protcoords(10,1)%fl,time
          !do l = p,f
          !write(6,*) 'vector'
          !if(time == 1026) write(6,*) 'hhhheeee',protcoords(13,1)%bm,protcoords(8,4)%am
...
31           if(clusmove .eqv. .false.) continue
           ...
           subroutine updateclusbonds(clsize,cllist,clno)
             integer :: m,rb,f,l,maxl,bdir,g,maxlengthss
             integer,intent(inout)::clsize
             integer,dimension(:),intent(inout) ::cllist,clno
             type(prottemp),dimension(:),allocatable:: tempcoord
             integer,dimension(:),allocatable:: tbo
             logical :: adjver
             double precision :: deltaenergy,initialenergy
             allocate(tempcoord(maxlength))
             allocate(tbo(maxlength))
