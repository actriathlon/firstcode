...
open(161,file='dimens.dat',action='write')
  ...
  allocate(vdist(4*clientlength,fullgridsize))
allocate(clusdist(2*mtype,fullgridsize))
allocate(loc(2))
allocate(cell(gridsize,gridsize,gridsize))
allocate(bindinghist(clientlength+1))
allocate(zloc(fullgridsize))
...
maxl = chlen(m)
do l=1,maxl
if((protcoords(m,l)%x>gridsize) .and. (protcoords(m,l)%x<(2*gridsize)+1) .and. (protcoords(m,l)%y > gridsize) &
.and. (protcoords(m,l)%y<(2*gridsize)+1) .and. (protcoords(m,l)%z  > gridsize) .and. (protcoords(m,l)%z< (2*gridsize)+1)) then
        cell(protcoords(m,l)%x-gridsize,protcoords(m,l)%y-gridsize,protcoords(m,l)%z-gridsize)%bm = m
        cell(protcoords(m,l)%x-gridsize,protcoords(m,l)%y-gridsize,protcoords(m,l)%z-gridsize)%bl = l
end if
...
15  continue
    ...
    tempcoord(l)%x = modulo(protcoords(m,l)%x+dx-1,fullgridsize)+1
    tempcoord(l)%y = modulo(protcoords(m,l)%y+dy-1,fullgridsize)+1
    tempcoord(l)%z = modulo(protcoords(m,l)%z+dz-1,fullgridsize)+1


else if (tele < 0.0) then

         tempcoord(l)%x = int(ran2(seed)*fullgridsize)+1
         tempcoord(l)%y = int(ran2(seed)*fullgridsize)+1
         tempcoord(l)%z = int(ran2(seed)*fullgridsize)+1
...
if((tempcoord(l)%x>gridsize).and. (tempcoord(l)%x<(2*gridsize)+1) .and. (tempcoord(l)%y>gridsize) .and. &
(tempcoord(l)%y < (2*gridsize)+1) .and. (tempcoord(l)%z>gridsize) .and. (tempcoord(l)%z<(2*gridsize)+1)) then  
if(cell(tempcoord(l)%x-gridsize,tempcoord(l)%y-gridsize,tempcoord(l)%z-gridsize)%bm /=0) then
rac = .false.
goto 37
end if
end if
    ...
    call removeenergyinter(m,l,deltaenergy,tempcoord)
                ...
                if((tempcoord(l)%x > gridsize-1) .and.(tempcoord(l)%x<((2*gridsize)+2)) .and. (tempcoord(l)%y > gridsize-1) &
 .and.(tempcoord(l)%y<((2*gridsize)+2)) .and. (tempcoord(l)%z > gridsize-1) .and.(tempcoord(l)%z<((2*gridsize)+2)) ) then


                if((tempcoord(l)%y > gridsize) .and.(tempcoord(l)%y<((2*gridsize)+1))&
 .and. (tempcoord(l)%z > gridsize) .and.(tempcoord(l)%z<((2*gridsize)+1)) ) then

if(tempcoord(l)%x < 2*gridsize) then

if(cell(tempcoord(l)%x-(gridsize-1),tempcoord(l)%y-gridsize,tempcoord(l)%z-gridsize)%bm/=0) then
                pr=cell(tempcoord(l)%x-(gridsize-1),tempcoord(l)%y-gridsize,tempcoord(l)%z-gridsize)%bm
                str=cell(tempcoord(l)%x-(gridsize-1),tempcoord(l)%y-gridsize,tempcoord(l)%z-gridsize)%bl
        bdir = -1
...
if(cell(tempcoord(l)%x-(gridsize+1),tempcoord(l)%y-gridsize,tempcoord(l)%z-gridsize)%bm/=0) then
        ...
        bdir = 1
                ...
                if((tempcoord(l)%x > gridsize) .and.(tempcoord(l)%x<((2*gridsize)+1))&
 .and. (tempcoord(l)%z > gridsize) .and.(tempcoord(l)%z<((2*gridsize)+1)) ) then


if(tempcoord(l)%y < 2*gridsize) then


        if(cell(tempcoord(l)%x-gridsize,tempcoord(l)%y-(gridsize-1),tempcoord(l)%z-gridsize)%bm/=0) then
                pr=cell(tempcoord(l)%x-gridsize,tempcoord(l)%y-(gridsize-1),tempcoord(l)%z-gridsize)%bm
                str=cell(tempcoord(l)%x-gridsize,tempcoord(l)%y-(gridsize-1),tempcoord(l)%z-gridsize)%bl
        bdir = -2
                      call bondforminter(m,pr,l,str,tempcoord,tbo,bdir,deltaenergy)
end if
end if

                if(tempcoord(l)%y > (gridsize+1)) then
        ...
        bdir = 2
                ...
                if((tempcoord(l)%y > gridsize) .and.(tempcoord(l)%y<((2*gridsize)+1))&
 .and. (tempcoord(l)%x > gridsize) .and.(tempcoord(l)%x<((2*gridsize)+1)) ) then


                if(tempcoord(l)%z < 2*gridsize) then

        if(cell(tempcoord(l)%x-gridsize,tempcoord(l)%y-gridsize,tempcoord(l)%z-(gridsize-1))%bm/=0) then
                pr=cell(tempcoord(l)%x-gridsize,tempcoord(l)%y-gridsize,tempcoord(l)%z-(gridsize-1))%bm
                str=cell(tempcoord(l)%x-gridsize,tempcoord(l)%y-gridsize,tempcoord(l)%z-(gridsize-1))%bl
        bdir = -3
                      call bondforminter(m,pr,l,str,tempcoord,tbo,bdir,deltaenergy)
end if
end if

                if(tempcoord(l)%z > (gridsize+1)) then
...
35    continue
         l = 1
         i = int(ran2(seed)*fullgridsize)+1
         j = int(ran2(seed)*fullgridsize)+1
         k = int(ran2(seed)*fullgridsize)+1
         !write(6,*) 'e'
...
if((i>gridsize).and. (i<(2*gridsize)+1) .and. (j>gridsize) .and. &
(j < (2*gridsize)+1) .and. (k>gridsize) .and. (k<(2*gridsize)+1)) then
if(cell(i-gridsize,j-gridsize,k-gridsize)%bm /=0) then
return
end if
end if
...
!write(6,*) 'linkerlen',protcoords(m,l)%linker
               ...
               if(sqrt(real((dx**2) + (dy**2) + (dz**2))) > protcoords(m,l-1)%linker) goto 15
                     ...
                     i = modulo(xbk+dx-1,fullgridsize)+1
                     j = modulo(ybk +dy-1,fullgridsize)+1
                     k  = modulo(zbk+dz-1,fullgridsize)+1
...
!phasecountdoub = phasecountdoub+1
       ...
       do l = 1,maxl
          ...
          dx = min(abs(protcoords(m,l)%x - dpcomclusterinit%x), fullgridsize - abs(protcoords(m,l)%x - dpcomclusterinit%x))
          dy = min(abs(protcoords(m,l)%y - dpcomclusterinit%y), fullgridsize - abs(protcoords(m,l)%y - dpcomclusterinit%y))
          dz = min(abs(protcoords(m,l)%z - dpcomclusterinit%z), fullgridsize - abs(protcoords(m,l)%z - dpcomclusterinit%z))
...
if(left .eqv. .false.) then
       ...
       maxl = chlen(m)
          dx = min(abs(protcoords(m,l)%x - ((3.0/4.0)*gridsize)), fullgridsize - abs(protcoords(m,l)%x - ((3.0/4.0)*gridsize)))
          dy = min(abs(protcoords(m,l)%y - dpcomclusterinit%y), fullgridsize - abs(protcoords(m,l)%y - (gridsize/2.0)))
          dz = min(abs(protcoords(m,l)%z - dpcomclusterinit%z), fullgridsize - abs(protcoords(m,l)%z - (gridsize/2.0)))
   ...
   vdist((2*clientlength)+1,lx+1) = vdist((2*clientlength)+1,lx+1) + 1.0
       ...
       maxl = chlen(m)
          dx = min(abs(protcoords(m,l)%x - (gridsize/2.0)), fullgridsize - abs(protcoords(m,l)%x - ((3.0/4.0)*gridsize)))
          dy = min(abs(protcoords(m,l)%y - dpcomclusterinit%y), fullgridsize - abs(protcoords(m,l)%y - (gridsize/2.0)))
          dz = min(abs(protcoords(m,l)%z - dpcomclusterinit%z), fullgridsize - abs(protcoords(m,l)%z - (gridsize/2.0)))
...
call comfind(m,.false.)
  if((com(1,m)%x >=gridsize+1.0) .and. (com(1,m)%x<=(2*gridsize)) .and. (com(1,m)%y >=gridsize+1.0) &
 .and. (com(1,m)%y<=(2*gridsize)) .and. (com(1,m)%z >=gridsize+1.0) .and. (com(1,m)%z<=(2*gridsize))) then
     loc(2) = loc(2)+1
	lc = .false.
...
zloc(protcoords(m,l)%x) = zloc(protcoords(m,l)%x) + 1
   if(protcoords(m,l)%am/=0) then
      g = protcoords(m,l)%am
      f = protcoords(m,l)%al
      if((bonddd(m,l) == 1) .and. (bonddd(g,f) ==-1) .and. &
(interenergy(protcoords(m,l)%type,protcoords(g,f)%type)< 0.0)) then
...
!call rdf(m,l)
 ...
 if((protcoords(g,f)%x >=gridsize+1.0) .and. (protcoords(g,f)%x<=(2*gridsize)) .and. (protcoords(g,f)%y >=gridsize+1.0) &
 .and. (protcoords(g,f)%y<=(2*gridsize)) .and. (protcoords(g,f)%z >=gridsize+1.0) .and. (protcoords(g,f)%z<=(2*gridsize))) then
 locb(2) = locb(2) +1
else
 locb(1) = locb(1) +1
end if
...
!call rdf(m,l)             
 ...
 locb(1) = locb(1) +1
    ...
    if(protcoords(m,l)%cm/=0) then
      g = protcoords(m,l)%cm
      f = protcoords(m,l)%cl
      if((bonddd(m,l) == 2) .and. (bonddd(g,f) ==-2) .and. &
(interenergy(protcoords(m,l)%type,protcoords(g,f)%type)< 0.0)) then
...
!call rdf(m,l)
 ...
 locb(1) = locb(1) +1
    ...
    if(protcoords(m,l)%em/=0) then
      g = protcoords(m,l)%em
      f = protcoords(m,l)%el
      if((bonddd(m,l) == 3) .and. (bonddd(g,f) ==-3) .and. &
(interenergy(protcoords(m,l)%type,protcoords(g,f)%type)< 0.0)) then
...
if(study(a,g) ==0) unbound(a,g) =unbound(a,g)+1
 ...
 !call comlargecomp(clsize,dummycom,checklist,route,rcounter,comx,comy,comz,toolarge)
    ...
    do m = 1,nprotein
       maxl = chlen(m)
       do l = 1,maxl
          sumsq%x = min(abs(protcoords(m,l)%x-comcluster%x),fullgridsize-abs(protcoords(m,l)%x-comcluster%x))
          sumsq%y = min(abs(protcoords(m,l)%y-comcluster%y),fullgridsize-abs(protcoords(m,l)%y-comcluster%y))
          sumsq%z = min(abs(protcoords(m,l)%z-comcluster%z),fullgridsize-abs(protcoords(m,l)%z-comcluster%z))
    ...
    do g = 1,clsize
       ...
       do l = 1,maxl
          ...
          if(((protcoords(m,l)%x-comcluster%x)>maxx) .and. &
               ((protcoords(m,l)%x-comcluster%x)<(fullgridsize/2))) maxx =(protcoords(m,l)%x-comcluster%x)
          if((((fullgridsize+protcoords(m,l)%x)-comcluster%x)>maxx).and. &
               (((fullgridsize+protcoords(m,l)%x)-comcluster%x)<(fullgridsize/2))) then
          maxx =((fullgridsize+protcoords(m,l)%x)-comcluster%x)
       end if
       if(((comcluster%x-protcoords(m,l)%x)>minx) .and. &
           ((comcluster%x-protcoords(m,l)%x)<(fullgridsize/2))) minx =(comcluster%x-protcoords(m,l)%x)
          if((((fullgridsize+comcluster%x)-protcoords(m,l)%x)>minx).and. &
               (((fullgridsize+comcluster%x)-protcoords(m,l)%x)<(fullgridsize/2))) then
          minx =((fullgridsize+comcluster%x)-protcoords(m,l)%x)
       end if       

       if(((protcoords(m,l)%y-comcluster%y)>maxy) .and. &
           ((protcoords(m,l)%y-comcluster%y)<(fullgridsize/2))) maxy =(protcoords(m,l)%y-comcluster%y)
       if((((fullgridsize+protcoords(m,l)%y)-comcluster%y)>maxy).and. &
            (((fullgridsize+protcoords(m,l)%y)-comcluster%y)<(fullgridsize/2))) then
          maxy =((fullgridsize+protcoords(m,l)%y)-comcluster%y)
       end if
       if(((comcluster%y-protcoords(m,l)%y)>miny) .and. &
            ((comcluster%y-protcoords(m,l)%y)<(fullgridsize/2))) miny =(comcluster%y-protcoords(m,l)%y)
       if((((fullgridsize+comcluster%y)-protcoords(m,l)%y)>miny).and. &
            (((fullgridsize+comcluster%y)-protcoords(m,l)%y)<(fullgridsize/2))) then
          miny =((fullgridsize+comcluster%y)-protcoords(m,l)%y)
       end if

       if(((protcoords(m,l)%z-comcluster%z)>maxz) &
            .and. ((protcoords(m,l)%z-comcluster%z)<(fullgridsize/2))) maxz =(protcoords(m,l)%z-comcluster%z)
       if((((fullgridsize+protcoords(m,l)%z)-comcluster%z)>maxz).and. &
            (((fullgridsize+protcoords(m,l)%z)-comcluster%z)<(fullgridsize/2))) then
          maxz =((fullgridsize+protcoords(m,l)%z)-comcluster%z)
       end if
       if(((comcluster%z-protcoords(m,l)%z)>minz) &
            .and. ((comcluster%z-protcoords(m,l)%z)<(gridsize/2))) minz =(comcluster%z-protcoords(m,l)%z)
       if((((fullgridsize+comcluster%z)-protcoords(m,l)%y)>minz).and. &
            (((fullgridsize+comcluster%z)-protcoords(m,l)%z)<(fullgridsize/2))) then
          minz =((fullgridsize+comcluster%z)-protcoords(m,l)%z)
       end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do t = 1,freezetime-1
     ...
     do m = nprotein+1-(topchains+nclients),nprotein-nclients,1
        ...
        !write(6,*) BIN
        read(121,*) BIN,bondx,bondy,bondz
...
close(108)
  ...
  subroutine rotate(m,l,b,tempcoords,xx,xy,xz,yx,yy,yz,zx,zy,zz,dx,dy,dz)
    ...
    !write(6,*) dx,dy,dz
    tempcoords(b)%x = modulo(protcoords(m,l)%x + (xx*dx) + (xy*dy) + (xz*dz) -1,fullgridsize)+1
    tempcoords(b)%y = modulo(protcoords(m,l)%y + (yx*dx) + (yy*dy) + (yz*dz)-1,fullgridsize)+1
    tempcoords(b)%z = modulo(protcoords(m,l)%z + (zx*dx) + (zy*dy) + (zz*dz)-1,fullgridsize)+1
...
if(l>1) then
    dx = min(abs(tempcoord(l)%x - protcoords(m,l-1)%x),fullgridsize-abs(tempcoord(l)%x &
         - protcoords(m,l-1)%x)) 
    dy = min(abs(tempcoord(l)%y - protcoords(m,l-1)%y),fullgridsize- abs(tempcoord(l)%y &
         - protcoords(m,l-1)%y)) 
    dz = min(abs(tempcoord(l)%z - protcoords(m,l-1)%z),fullgridsize - abs(tempcoord(l)%z &
         - protcoords(m,l-1)%z)) 
...
19     continue
       ...
       tempcoord(1)%x = modulo(protcoords(m,1)%x + dx-1,fullgridsize)+1
       tempcoord(1)%y = modulo(protcoords(m,1)%y + dy-1,fullgridsize)+1
       tempcoord(1)%z = modulo(protcoords(m,1)%z + dz-1,fullgridsize)+1

     dx = min(abs(tempcoord(1)%x - protcoords(m,2)%x),fullgridsize-abs(tempcoord(1)%x &
         - protcoords(m,2)%x)) 
    dy = min(abs(tempcoord(1)%y - protcoords(m,2)%y),fullgridsize- abs(tempcoord(1)%y &
         - protcoords(m,2)%y)) 
    dz = min(abs(tempcoord(1)%z - protcoords(m,2)%z),fullgridsize - abs(tempcoord(1)%z &
         - protcoords(m,2)%z)) 
...
if((tempcoord(1)%x>gridsize).and. (tempcoord(1)%x<(2*gridsize)+1) .and. (tempcoord(1)%y>gridsize) .and. &
(tempcoord(1)%y < (2*gridsize)+1) .and. (tempcoord(1)%z>gridsize) .and. (tempcoord(1)%z<(2*gridsize)+1)) then
if(cell(tempcoord(1)%x-gridsize,tempcoord(1)%y-gridsize,tempcoord(1)%z-gridsize)%bm /=0) then
reptcont = .false.
goto 83
end if
end if
 ...
 call bonddirection(tbo(1))
       ...
       !if(deltaenergy<0.0)write(6,*) 'energy post intra=',deltaenergy
                ...
                if((tempcoord(1)%x > gridsize-1) .and.(tempcoord(1)%x<((2*gridsize)+2)) .and. (tempcoord(1)%y > gridsize-1) &
.and.(tempcoord(1)%y<((2*gridsize)+2)) .and. (tempcoord(1)%z > gridsize-1) .and.(tempcoord(1)%z<((2*gridsize)+2)) ) then


                if((tempcoord(1)%y > gridsize) .and.(tempcoord(1)%y<((2*gridsize)+1))&
 .and. (tempcoord(1)%z > gridsize) .and.(tempcoord(1)%z<((2*gridsize)+1)) ) then

if(tempcoord(1)%x < 2*gridsize) then

if(cell(tempcoord(1)%x-(gridsize-1),tempcoord(1)%y-gridsize,tempcoord(1)%z-gridsize)%bm/=0) then
                pr=cell(tempcoord(1)%x-(gridsize-1),tempcoord(1)%y-gridsize,tempcoord(1)%z-gridsize)%bm
                str=cell(tempcoord(1)%x-(gridsize-1),tempcoord(1)%y-gridsize,tempcoord(1)%z-gridsize)%bl
        bdir = -1
                      call bondforminter(m,pr,1,str,tempcoord,tbo,bdir,deltaenergy)
end if
end if
...
if(cell(tempcoord(1)%x-(gridsize+1),tempcoord(1)%y-gridsize,tempcoord(1)%z-gridsize)%bm/=0) then
        ...
        bdir = 1
                ...
                if((tempcoord(1)%x > gridsize) .and.(tempcoord(1)%x<((2*gridsize)+1))&
 .and. (tempcoord(1)%z > gridsize) .and.(tempcoord(1)%z<((2*gridsize)+1)) ) then


if(tempcoord(1)%y < 2*gridsize) then


        if(cell(tempcoord(1)%x-gridsize,tempcoord(1)%y-(gridsize-1),tempcoord(1)%z-gridsize)%bm/=0) then
                pr=cell(tempcoord(1)%x-gridsize,tempcoord(1)%y-(gridsize-1),tempcoord(1)%z-gridsize)%bm
                str=cell(tempcoord(1)%x-gridsize,tempcoord(1)%y-(gridsize-1),tempcoord(1)%z-gridsize)%bl
        bdir = -2
                      call bondforminter(m,pr,1,str,tempcoord,tbo,bdir,deltaenergy)
end if
end if

                if(tempcoord(1)%y > (gridsize+1)) then
        ...
        bdir = 2
                ...
                if((tempcoord(1)%y > gridsize) .and.(tempcoord(1)%y<((2*gridsize)+1))&
 .and. (tempcoord(1)%x > gridsize) .and.(tempcoord(1)%x<((2*gridsize)+1)) ) then


                if(tempcoord(1)%z < 2*gridsize) then

        if(cell(tempcoord(1)%x-gridsize,tempcoord(1)%y-gridsize,tempcoord(1)%z-(gridsize-1))%bm/=0) then
                pr=cell(tempcoord(1)%x-gridsize,tempcoord(1)%y-gridsize,tempcoord(1)%z-(gridsize-1))%bm
                str=cell(tempcoord(1)%x-gridsize,tempcoord(1)%y-gridsize,tempcoord(1)%z-(gridsize-1))%bl
        bdir = -3
                      call bondforminter(m,pr,1,str,tempcoord,tbo,bdir,deltaenergy)
end if
end if

                if(tempcoord(1)%z > (gridsize+1)) then
...
15     continue
       ...
       tempcoord(maxl)%x = modulo(protcoords(m,maxl)%x + dx-1,fullgridsize)+1
       tempcoord(maxl)%y = modulo(protcoords(m,maxl)%y + dy-1,fullgridsize)+1
       tempcoord(maxl)%z = modulo(protcoords(m,maxl)%z + dz-1,fullgridsize)+1

     dx = min(abs(tempcoord(maxl)%x - protcoords(m,maxl-1)%x),fullgridsize-abs(tempcoord(maxl)%x &
         - protcoords(m,maxl-1)%x)) 
    dy = min(abs(tempcoord(maxl)%y - protcoords(m,maxl-1)%y),fullgridsize- abs(tempcoord(maxl)%y &
         - protcoords(m,maxl-1)%y)) 
    dz = min(abs(tempcoord(maxl)%z - protcoords(m,maxl-1)%z),fullgridsize - abs(tempcoord(maxl)%z &
...
if((tempcoord(maxl)%x>gridsize).and. (tempcoord(maxl)%x<(2*gridsize)+1) .and. (tempcoord(maxl)%y>gridsize) .and. &
(tempcoord(maxl)%y < (2*gridsize)+1) .and. (tempcoord(maxl)%z>gridsize) .and. (tempcoord(maxl)%z<(2*gridsize)+1)) then
if(cell(tempcoord(maxl)%x-gridsize,tempcoord(maxl)%y-gridsize,tempcoord(maxl)%z-gridsize)%bm /=0) then
goto 83
end if
end if
 ...
 call bonddirection(tbo(maxl))
       ...
       do st = 2,maxl-2
        ...
        if((tempcoord(maxl)%x > gridsize-1) .and. (tempcoord(maxl)%x<((2*gridsize)+2)) .and. (tempcoord(maxl)%y > gridsize-1) &
 .and.(tempcoord(maxl)%y<((2*gridsize)+2)) .and. (tempcoord(maxl)%z > gridsize-1) .and.(tempcoord(maxl)%z<(2*gridsize)+2) ) then


                if((tempcoord(maxl)%y > gridsize) .and.(tempcoord(maxl)%y<((2*gridsize)+1))&
 .and. (tempcoord(maxl)%z > gridsize) .and.(tempcoord(maxl)%z<((2*gridsize)+1)) ) then

if(tempcoord(maxl)%x < 2*gridsize) then

if(cell(tempcoord(maxl)%x-(gridsize-1),tempcoord(maxl)%y-gridsize,tempcoord(maxl)%z-gridsize)%bm/=0) then
                pr=cell(tempcoord(maxl)%x-(gridsize-1),tempcoord(maxl)%y-gridsize,tempcoord(maxl)%z-gridsize)%bm
                str=cell(tempcoord(maxl)%x-(gridsize-1),tempcoord(maxl)%y-gridsize,tempcoord(maxl)%z-gridsize)%bl
        bdir = -1
                      call bondforminter(m,pr,maxl,str,tempcoord,tbo,bdir,deltaenergy)
end if
end if
...
if(cell(tempcoord(maxl)%x-(gridsize+1),tempcoord(maxl)%y-gridsize,tempcoord(maxl)%z-gridsize)%bm/=0) then
        ...
        bdir = 1
                ...
                if((tempcoord(maxl)%x > gridsize) .and.(tempcoord(maxl)%x<((2*gridsize)+1))&
 .and. (tempcoord(maxl)%z > gridsize) .and.(tempcoord(maxl)%z<((2*gridsize)+1)) ) then


if(tempcoord(maxl)%y < 2*gridsize) then


        if(cell(tempcoord(maxl)%x-gridsize,tempcoord(maxl)%y-(gridsize-1),tempcoord(maxl)%z-gridsize)%bm/=0) then
                pr=cell(tempcoord(maxl)%x-gridsize,tempcoord(maxl)%y-(gridsize-1),tempcoord(maxl)%z-gridsize)%bm
                str=cell(tempcoord(maxl)%x-gridsize,tempcoord(maxl)%y-(gridsize-1),tempcoord(maxl)%z-gridsize)%bl
        bdir = -2
                      call bondforminter(m,pr,maxl,str,tempcoord,tbo,bdir,deltaenergy)
end if
end if

                if(tempcoord(maxl)%y > (gridsize+1)) then
        ...
        bdir = 2
                ...
                if((tempcoord(maxl)%y > gridsize) .and.(tempcoord(maxl)%y<((2*gridsize)+1))&
 .and. (tempcoord(maxl)%x > gridsize) .and.(tempcoord(maxl)%x<((2*gridsize)+1)) ) then


                if(tempcoord(maxl)%z < 2*gridsize) then

        if(cell(tempcoord(maxl)%x-gridsize,tempcoord(maxl)%y-gridsize,tempcoord(maxl)%z-(gridsize-1))%bm/=0) then
                pr=cell(tempcoord(maxl)%x-gridsize,tempcoord(maxl)%y-gridsize,tempcoord(maxl)%z-(gridsize-1))%bm
                str=cell(tempcoord(maxl)%x-gridsize,tempcoord(maxl)%y-gridsize,tempcoord(maxl)%z-(gridsize-1))%bl
        bdir = -3
                      call bondforminter(m,pr,maxl,str,tempcoord,tbo,bdir,deltaenergy)
end if
end if

                if(tempcoord(maxl)%z > (gridsize+1)) then
...
!write(6,*) 'ENERGYSUBROUTINE',totalenergy
  ...
  subroutine adjacent(pr,ch1,ch2,tempory,adjver,bdir)
    ...
    if((abs(delx) == 1) .or. (abs(delx) == (fullgridsize - 1))) then
       dx = 1
       bdir = delx
    else if(delx == 0) then
       dx = 0
    ...
    if((abs(dely) == 1) .or.(abs(dely) == (fullgridsize - 1))) then
       dy = 1
       bdir = 2*dely
    else if(dely == 0) then
       dy = 0
    ...
    if((abs(delz) == 1) .or. (abs(delz) == (fullgridsize -1))) then
       dz = 1
       bdir = 3*delz
    else if(delz == 0) then
       dz = 0
...
!linkerlength = 7
        ...
        do l = 2,maxl,1
           ...
           if (dcomx >protcoords(m,1)%linker) dcomx = dcomx-fullgridsize
           if (dcomx <(-protcoords(m,1)%linker)) dcomx = fullgridsize+dcomx
           comx = comx + ((maxl+1-l)*dcomx)
           dcomy = protcoords(m,l)%y-protcoords(m,l-1)%y
           !if(abs(dcomy) > 7) write(6,*) 'over pbc',fullgridsize,linkerlength,dcomy
           if (dcomy >protcoords(m,1)%linker) dcomy = dcomy-fullgridsize
           if (dcomy <(-protcoords(m,1)%linker)) dcomy = fullgridsize+dcomy
           ...
           if (dcomz >protcoords(m,1)%linker) dcomz = dcomz-fullgridsize
           if (dcomz <(-protcoords(m,1)%linker)) dcomz = fullgridsize+dcomz
           comz = comz + ((maxl+1-l)*dcomz)
        end do
        !write(6,*) 'deltas',comx,comy,comz
        com(1,m)%x = modulo(protcoords(m,1)%x +(real(comx)/(maxl)) -1,real(fullgridsize))+1
        com(1,m)%y = modulo(protcoords(m,1)%y +(real(comy)/(maxl)) -1,real(fullgridsize))+1
        com(1,m)%z = modulo(protcoords(m,1)%z +(real(comz)/(maxl)) -1,real(fullgridsize))+1
        !end do
  ...
  subroutine tracking(m)
    ...
    double precision :: dcx,dcy,dcz
         ...
         dcx = com(1,m)%x-com(2,m)%x
          if (dcx > fullgridsize/2) dcx = dcx-fullgridsize
          if (dcx < -fullgridsize/2) dcx = fullgridsize + dcx
          trackx = trackx + dcx
          dcy = com(1,m)%y-com(2,m)%y
          if (dcy > fullgridsize/2) dcy = dcy - fullgridsize
          if (dcy < -fullgridsize/2) dcy = dcy +fullgridsize
          tracky = tracky + dcy
          dcz = com(1,m)%z-com(2,m)%z
          if (dcz > fullgridsize/2) dcz = dcz - fullgridsize
          if (dcz < -fullgridsize/2) dcz = dcz + fullgridsize
          trackz = trackz + dcz
  ...
  subroutine rms(m)
    ...
    msdx = (min(abs(com(1,m)%x - com(2,m)%x), &
         (fullgridsize - abs(com(1,m)%x -com(2,m)%x))))**2
    msdy = (min(abs(com(1,m)%y -com(2,m)%y), &
         (fullgridsize - abs(com(1,m)%y -com(2,m)%y))))**2
    msdz = (min(abs(com(1,m)%z - com(2,m)%z), &
         (fullgridsize - abs(com(1,m)%z -com(2,m)%z))))**2
...
!avez = avez/maxl
       ...
       do l = 1, maxl,1
          rog = (min(modulo(protcoords(m,l)%x-com(1,m)%x,fullgridsize*1.0),(fullgridsize - &
               modulo(protcoords(m,l)%x-com(1,m)%x,fullgridsize*1.0))))**2+ &
               (min(modulo(protcoords(m,l)%y-com(1,m)%y,fullgridsize*1.0),(fullgridsize - &
               modulo(protcoords(m,l)%y-com(1,m)%y,fullgridsize*1.0))))**2+ &
               (min(modulo(protcoords(m,l)%z-com(1,m)%z,fullgridsize*1.0),(fullgridsize - &
               modulo(protcoords(m,l)%z-com(1,m)%z,fullgridsize*1.0))))**2
          totrog = totrog + rog
       end do
 ...
 noROGS = noROGS+1
  ...
  subroutine length
    ...
    do m = 1,nprotein,1
        ...
        maxl = chlen(m)
        dxsum = min(abs(protcoords(m,maxl)%x - protcoords(m,1)%x),fullgridsize-abs(protcoords(m,maxl)%x - protcoords(m,1)%x))
        dysum = min(abs(protcoords(m,maxl)%y - protcoords(m,1)%y),fullgridsize-abs(protcoords(m,maxl)%y - protcoords(m,1)%y))
        dzsum = min(abs(protcoords(m,maxl)%z - protcoords(m,1)%z),fullgridsize-abs(protcoords(m,maxl)%z - protcoords(m,1)%z))
...
polymerl = polymerl + normchainl
  ...
  subroutine dataout
    ...
    write(choice,*) 'pbc', real(4*fullgridsize),real(4*fullgridsize),real(4*fullgridsize)
    ...
    do m = 1,nprotein,1
       ...
       do l = 1,maxl,1
          ...
          if(bonddd(m,l) == 1) bondposit(m,l)%x = modulo(bondposit(m,l)%x + 0.5,real(fullgridsize))
          if(bonddd(m,l) == -1) bondposit(m,l)%x = modulo(bondposit(m,l)%x - 0.5,real(fullgridsize))
          if(bonddd(m,l) == 2) bondposit(m,l)%y = modulo(bondposit(m,l)%y + 0.5,real(fullgridsize))
          if(bonddd(m,l) == -2) bondposit(m,l)%y = modulo(bondposit(m,l)%y - 0.5,real(fullgridsize))
          if(bonddd(m,l) == 3) bondposit(m,l)%z = modulo(bondposit(m,l)%z + 0.5,real(fullgridsize))
          if(bonddd(m,l) == -3) bondposit(m,l)%z = modulo(bondposit(m,l)%z - 0.5,real(fullgridsize))
...
if(m>nscaffold) then
 dx = min(abs(protcoords(m,l)%x - protcoords(m,l-1)%x),fullgridsize-abs(protcoords(m,l)%x &
                  - protcoords(m,l-1)%x))
             dy = min(abs(protcoords(m,l)%y - protcoords(m,l-1)%y),fullgridsize-abs(protcoords(m,l)%y &
                  - protcoords(m,l-1)%y))
             dz = min(abs(protcoords(m,l)%z - protcoords(m,l-1)%z),fullgridsize-abs(protcoords(m,l)%z &
                  - protcoords(m,l-1)%z))
          ...
          do a = 1,clsize-1,1
             ...
             do g = a+1,clsize
                ...
                if(cb(b,f)==1) then
                   ...
                   if (delx > (fullgridsize)/2) delx = delx-fullgridsize
                   if (delx < -(fullgridsize)/2) delx = delx + fullgridsize
                   comx(b,f) =  delx
                   comx(f,b) = -delx
                   dely = com(1,f)%y - com(1,b)%y
                   if (dely > (fullgridsize)/2) dely = dely-fullgridsize
                   if (dely < -(fullgridsize)/2) dely = dely + fullgridsize
                   comy(b,f) =  dely
                   comy(f,b) = -dely
                   delz = com(1,f)%z - com(1,b)%z
                   if (delz > (fullgridsize)/2) delz = delz-fullgridsize
                   if (delz < -(fullgridsize)/2) delz = delz + fullgridsize
                   comz(b,f) =  delz
                   comz(f,b) = -delz
                end if
...
76        if(clsize == 1) then
          ...
          !write(6,*) dummycom%x
          comcluster%x = INT(modulo((com(1,centralchain)%x) + (dummycom%x/totcluspop)-1,real(fullgridsize))+1)
          comcluster%y = INT(modulo((com(1,centralchain)%y) + (dummycom%y/totcluspop)-1,real(fullgridsize))+1)
          comcluster%z = INT(modulo((com(1,centralchain)%z) + (dummycom%z/totcluspop)-1,real(fullgridsize))+1)


          dpcomcluster%x = modulo((com(1,centralchain)%x) + (dummycom%x/totcluspop)-1,real(fullgridsize))+1
          dpcomcluster%y = modulo((com(1,centralchain)%y) + (dummycom%y/totcluspop)-1,real(fullgridsize))+1
          dpcomcluster%z = modulo((com(1,centralchain)%z) + (dummycom%z/totcluspop)-1,real(fullgridsize))+1
...
comcluster%x = gridsize/2
comcluster%y = gridsize/2
comcluster%z = gridsize/2
...
subroutine leftdistribution(m,l)
   ...
   integer,intent(in)::m,l
    ...
    double precision :: delta
          ...
          !phasecount = phasecount+1
          dx = min(abs(protcoords(m,l)%x - (gridsize/2.0)), fullgridsize - abs(protcoords(m,l)%x - (fullgridsize/2.0)))
          dy = min(abs(protcoords(m,l)%y - (gridsize/2.0)), fullgridsize - abs(protcoords(m,l)%y - (fullgridsize/2.0)))
          dz = min(abs(protcoords(m,l)%z - (gridsize/2.0)), fullgridsize - abs(protcoords(m,l)%z - (fullgridsize/2.0)))
